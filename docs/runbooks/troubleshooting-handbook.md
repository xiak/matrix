...
| Credential recovery succeeds, but the gate stops at `credential-recovery-forced-change-only` | Compare the protected route used by the gate with both the current and exact predecessor OpenAPI documents | The gate called a nonexistent `/organizations` route and received 404 instead of exercising authorization. Call the published `/accounts` route and require exactly 403 while password replacement is mandatory; never accept an arbitrary failure status as proof of denial. |
