...
| Foundation `devops/infrastructure/remoteexecution/ssh.py` | `REJECT` for enrollment and node runtime | Arbitrary script input, inline private keys, optional insecure checking and post-buffer output bounds do not meet the resident-node boundary. The current product supports only the one-time registration command; any independently managed SSH access remains outside Matrix and is not an accepted bootstrap, rescue or enrollment path. |
