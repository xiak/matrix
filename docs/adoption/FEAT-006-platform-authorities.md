...
| Matrix shared password-attempt slice | `31c18531957ea6d2b52a8ac2cdcd084e13287031` | Fixed production source only; adapt its durable reservation and atomic consumption into this branch's authority shape, not its release profile, workflow or IAM documentation. |
| Matrix shared password-attempt fixture repair | `92e3073e290104089af3e02ddaaa6a7d8fb2c457` | Fixed test-only successor; retain the removed raw hash-lookup boundary and adapt only the existing Audit/IAM fixture owners. |
...
| `31c18531`: shared password reservation and atomic final consumption | `ADAPT` | Commit a bounded per-USER attempt before Argon2 work, keep the expensive verifier outside database locks, and consume the exact attempt/generation only in the final Session or password transaction. Remove API execution of raw `lookup_login`/`lookup_password` and old mutation overloads; unknown identities still perform bounded dummy verification without allocating rows. |
| `92e3073`: Audit/IAM restricted-role fixture repair | `ADAPT` tests only | Exercise the public reservation functions and prove raw resolver denial instead of restoring removed privileges. Do not import its checkpoint, IAM/009 evidence, unrelated self-session feature, or donor Audit/PaaS/profile values. |
