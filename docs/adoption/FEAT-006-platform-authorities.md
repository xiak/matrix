...
| Matrix `3080922f6ae1871f1c351d5ee30f03551fc3c605`, S1 session transaction, immutable completion and existing tests | `REUSE` actual Session, credential generation, identity locks and outbox; `ADAPT` an atomic other-session set and its exact completion; `REJECT` a loop of single-target commands | The single-target completion cannot represent an empty set or protect later logins from a retried bulk intent. A purpose-bound aggregate completion and immutable target references belong to the same SQL owner, not a generic receipt service or new Session model. Existing individual routes and historical evidence stay unchanged; source inspection does not establish S1b runtime acceptance. |
...
| Matrix UI `8e8b0f608827fb00c9a0e677e78ec12a7e047315`, own-session HTTP adapter, provider and FEAT-007; included by `ec5e832ad42cafea31cd731cfdbaa046b3537229` | `REFERENCE` fixed consumer of backend `3080922f`; `REJECT` inheriting preview/browser acceptance as real authority evidence | Read-only source confirms the closed list/revoke contract and retained original request identity. The peer's DEV evidence uses MOCK sessions and explicitly performs no real revoke. No UI code/assets or WIP are imported here. IAM/010 owns the still-unaccepted real dual-session integration boundary; subsequent navigation changes are not session acceptance. |
...
| Matrix `285706e3adf76fb0c109dad474f06266c8b67ab5`, original `Secret`, private TOTP/AccessKey codecs, `credentialBindingBytes`, CredentialIssuer and seed protection | `REUSE` strict private codec, bounded standard randomness, length framing and standard AES-GCM/HKDF primitives; `ADAPT` a separate email-verification purpose and complete immutable intent; `REJECT` using TOTP/AccessKey/recovery material or short-code plain digests | The new mail worker needs only the original address code, not factor secrets or identity authority. Eight-digit codes require durable attempts and original-intent consumption separately from encryption. Per-key versus keyset commitments stay distinct; pure file validation cannot assert historical registry, retirement, supported restore or current contact ownership. |
| Same fixed source, `000001_authority` password attempts and final Session/password mutations | `REUSE` committed shared debit and Account/USER/credential/Session lock order; `ADAPT` explicit purpose/intent binding when the contact transaction is admitted | Existing LOGIN/PASSWORD_CHANGE names cannot conceal email reauthentication. A fresh interface must not obtain another guessing budget or consume an unrelated prior reservation. This source inspection is not a new SQL permission or schema admission. |
| Matrix `8ccc632796727261563c952a89e4dfd3ce947144`, private mail codecs, EmailVerificationBinding and encrypted code material | `REUSE` the sole strict codecs and purpose-bound encryption; `ADAPT` actual bootstrap/keyset custody and immutable original verification in IAM's existing transaction owner | Ciphertext possession is not current Session eligibility or one-time consumption. The new contact/mail persistence is separate from Audit outbox delivery, with a dedicated least-privilege role; no temporary authentication Session, arbitrary message endpoint or duplicated crypto encoder is introduced. |
| Matrix `841ebe89aa55121ac4686dc469006ed10b47f0eb`, SMTP submission adapter and real Postfix gate | `REUSE` authenticated TLS, bounded transport and honest observation; `ADAPT` durable fenced attempts and a contact-confirmed notice; `REJECT` a second installation-ID grammar | Installation IDs remain governed by the existing IAM contract, including sealed historical IDs. A committed notification lease precedes network I/O; stable Message-ID and DATA 250 do not imply exactly-once delivery or final receipt. No mail credentials or powers are inherited by IAM's Audit worker. |
...
| Matrix `16b42679a3f19d73ee41af016e81ef2b95b938fd`, installation `localmachine/credentials.go`, `topology/topology.go` and `TestBackupBindsCurrentAccessKeyWrappingKeyWithoutArchivingIt` | `REFERENCE` write-once scope validation, restricted single-file mounts and nonsecret backup commitment; `REJECT` copying AccessKey's single-key format or inheriting MFA recovery evidence | Fixed code preserves the original bootstrap-bound AccessKey material rather than regenerating it, and checks a backup's material without archiving the keyring. This is not a TOTP codec, a same-snapshot multi-key custody summary, a replica-visible restore barrier or proof of current post-backup recovery qualification. TOTP's immutable per-key commitment must survive keyset revision/active-key changes; a separate set digest owns those changes. No installation files or foreign acceptance status are imported. |
...
| Matrix `b7a70bfa9e53f0a5f16619c60523c84613cb7b0b`, Account/USER/Session and closed StepUp contracts, product Profile/PDP and strict API codecs | `REUSE` the actual ACCOUNT resource, Session-held proof boundary, immutable system-policy versions and current contract owners; `ADAPT` exact security-settings intent and nonsecret historical completion | IAM/009 fixes the ordinary-user setting, before-change authorization and monotonic Session qualification target before this slice. Configuration is not factor state, authentication strength, a policy or a transferable permit. Historical completion describes its original caller, not a later reader's Session. `REJECT` a second security-policy model, selectors, generic merge/attributes, inferred false for missing MFA settings, automatic new rights on existing attachments or accepting a new StepUp operation before its transaction exists. No UI, installation profile or foreign acceptance is imported. |
