...
| Matrix personal-security enrollment console | `80225dce692921aaaeda12e96b58a464f931da30` | Fixed same-repository source only; adapt notification-contact, TOTP enrollment and one-time recovery-code semantics into the current owners without importing its i18n/preview architecture, generated assets, checkpoint or acceptance state. |
...
| Personal-security domain, repository parser, provider, settings renderer and recovery-code completion at `80225dce` | `ADAPT` | Consume the authenticated user's verified notification contact and first TOTP enrollment without a caller identity selector. Preserve one-time seed and recovery-code disclosure, equal-replay secrecy, session revocation after confirmation and explicit recovery-code acknowledgement. Implement them in the existing account settings and login state machine; reject donor i18n, preview repositories, generated assets and inherited acceptance. |
