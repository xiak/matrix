...
| Personal MFA lifecycle | Non-preview settings consume fixed first-TOTP enrollment state and commands; only first `APPLIED` can disclose provisioning in memory and confirmation requires exactly ten unique one-time codes before normal reauthentication. A lost start response now retains only the credential/account/user-bound requestId across content remounts, blocks a second write, and queries the exact original intent; `PENDING` can only be cancelled exactly, while `NOT_FOUND` and unavailable inspection remain UNKNOWN. The login surface consumes the separately bounded S2b self-recovery candidate: one old code irreversibly replaces the old factor through a `RECOVERY/ENROLLMENT` challenge without creating a Session. Recovery start uncertainty retains only the original non-secret request context; confirmation uncertainty never replays ten codes, and a fresh `LOGIN/RECOVER` can inspect a pre-commit loss instead of looping. The same provider is executable as a visibly labelled MOCK, with no LIVE failure fallback. Source and synchronized embed are pushed at `d7d6333d`; full ownership and remaining real-IAM acceptance are in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#受限自助-totp-恢复客户端的开发验收证据). |
