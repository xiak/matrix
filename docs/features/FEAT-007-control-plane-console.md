...
| Visual policy review | The shared LIVE/isolated-MOCK policy author presents one current statement's effect, exact Actions, resource selectors and conditions before optional full JSON; multiple statements switch through a labelled selector rather than mounting the full declaration. LIVE visual version publishing now reuses that summary and keeps current/proposed JSON comparison collapsible. Free-form JSON still displays the full source/comparison because a partial summary could omit unknown fields. A `390px` DEV creation-review path had no document overflow or new warning/error. Source and embed are pushed at `7d04fe84`; real-IAM browser acceptance remains open in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略版本读取与变更的开发验收证据). |
