...
IAM policy-version change slice.

| Gate | Evidence |
| --- | --- |
| IAM customer policy versions | The LIVE detail keeps lazy exact-version inspection and now reviews non-default set-default/retire in the content area, with a session-scoped unresolved-result panel. The explicit MOCK editor and usage view remain separate. Source and synchronized embed are pushed at `cd0be0bc`. The complete 43-file/707-case Vitest suite passed with two workers and a 10-second per-test budget; three normalization cases, type/lint/architecture/228-pair style gates, 40-page production export, 223-file embed equivalence and `go test -p 2 ./...` / `go vet -p 2 ./...` also passed. The unchanged default five-second timing gate is not claimed as passed. At 390px the retained MOCK version page had no document overflow or dialog; no real IAM write browser acceptance or cross-session unknown-result recovery was tested. Exact semantics and exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略版本读取与变更的开发验收证据). |
...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 707 Vitest cases across 43 files passed with two workers and a 10-second per-test budget, together with three static-normalization cases. The default five-second timing gate remains unclaimed. |
