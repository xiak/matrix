...
IAM policy-version publication slice.
...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 712 Vitest cases across 43 files passed with two workers and a 10-second per-test budget, together with three static-normalization cases. The default five-second timing gate remains unclaimed. |
