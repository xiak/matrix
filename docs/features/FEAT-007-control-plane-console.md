...
Verified through 2026-09-24 against the current all-service navigation and
IAM Role creation slices.

| Gate | Evidence |
| --- | --- |
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 674 Vitest cases across 42 files and three static-normalization cases passed with the existing five-second per-test timeout unchanged. |
| Theme and static-export boundary | 228 semantic contrast pairs passed across light, mixed and dark workspace/shell surfaces. Production preview was disabled; all 223 embedded files matched the normalized immutable export. |
...
| Personal MFA lifecycle | Non-preview settings consume fixed first-TOTP enrollment state and commands; only first `APPLIED` can disclose provisioning in memory and confirmation requires exactly ten unique one-time codes before normal reauthentication. A lost start response retains only the credential/account/user-bound requestId across content remounts, blocks a second write and queries the exact original intent; `PENDING` can only be cancelled exactly, while `NOT_FOUND` and unavailable inspection remain UNKNOWN. A bound factor now also consumes the fixed purpose-limited recovery-code regeneration contract. The content-area flow creates a two-minute step-up for the exact factor revision, verifies current password plus fresh TOTP, and submits regeneration with the original requestId. Unknown start permits only exact retry/read, unknown verification only reads the original step-up, and unknown regeneration only reads the original completion; password, OTP and write commands are never replayed. Only first `APPLIED` displays exactly ten unique codes in component memory, while `EQUAL_REPLAY` and by-request reads cannot return secrets. A normal re-login by the same Account/User can query the original non-secret regeneration with the fresh Session, but cannot reuse the old proof. The login surface separately retains the S2b recovery flow in which one old code irreversibly replaces the old factor without creating a Session. The same preview remains visibly labelled MOCK, with no LIVE failure fallback; account security-setting changes remain MOCK because their HTTP contract is not fixed. The complete 42-file/682-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. Source and synchronized embed are pushed at `a2ff4f6ff1fc56fa9a43ab56499962e03ac05733`; full ownership and remaining real-IAM acceptance are in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人-mfa-生命周期的开发验收证据). |
...
| Account security-settings preview | The settings workspace owns a separate semantic Account-security component instead of storing tenant rules inside the personal-factor flow. It defaults to a read-only rule summary, then uses explicit edit, exact-target review and scoped password-plus-TOTP step-up. Account-rule and User-SSO saves are separate preview commands; readable state does not imply update capability. Update denial and version conflict both discard operation-bound proof and secrets without replay. A lost save response persists the original intent as `UNKNOWN` across route remounts, suppresses success feedback and blocks a second write; a repository reject/timeout journals the same non-secret intent before exposing UNKNOWN. If both save and journal fail, a session-scoped `UNRECOVERABLE` write lock survives route navigation and successful workspace rereads, exposes no misleading query action and can only be reset by ending the isolated preview session; this is not a proposed LIVE recovery protocol. `NOT_FOUND` and unavailable inspection stay unknown, while only authoritative applied/rejected results unlock a queryable flow. The review preserves protected Root/platform-managed identities and tightening/relaxing session semantics. The 192-case targeted workspace/repository run, complete 609-case frontend run, 228-pair theme gate, 40-page export, 222-file embed equivalence, full Go test/vet and architecture/style gates passed. A `539px` DEV journey proved route-persistent locking, no dialog or page overflow and `clientWidth === scrollWidth === 539`; source and synchronized embedded UI are pushed at `2c460a3904daebcd5c00e4cb9683e89ab8b169b4`. Design and IAM acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#账号级-mfa-要求-mock-的开发验收证据). |
