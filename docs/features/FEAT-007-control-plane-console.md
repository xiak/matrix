...
IAM policy-version authoring and isolated first-enrollment preview slices.

| Gate | Evidence |
| --- | --- |
| IAM customer policy versions | LIVE detail retains lazy exact-version inspection and inline default/retirement review. Its new CUSTOMER/TENANT version author offers JSON plus a catalog-driven visual path for one exact Action per statement; unrepresentable JSON remains untouched, and catalog denial is local to the visual path. Publication remains separate from default selection and authorization. The explicit MOCK editor and usage view remain separate. Source and synchronized embed are pushed at `4426f9a5`. The retained-MOCK mobile inspection does not establish LIVE write-browser acceptance or cross-session unknown-result recovery. Exact semantics and exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略版本读取与变更的开发验收证据). |
| IAM customer policy creation | The LIVE content-area author reuses the JSON and catalog-driven visual editor, requires an explicit Action and exact instance target by default, then separates review, creation and attachment. The strict response parser accepts only the current Account's matching CUSTOMER/TENANT Policy and default document; uncertain transport/protocol outcomes retain the original request across IAM navigation, with byte-equivalent retry and explicit acknowledgement. The four-method MOCK author remains isolated. Source and synchronized embed are pushed at `ec3fc845`; real IAM browser and cross-login recovery remain unaccepted. Contract and UX evidence belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#live-自定义策略新建的开发验收证据). |
...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 727 Vitest cases across 44 files passed with two workers and a 10-second per-test budget, together with three static-normalization cases; 40-page production export, 223-file embed equivalence and `go test -p 2 ./...` / `go vet -p 2 ./...` also passed. The default five-second timing gate remains unclaimed. Current source and synchronized embed are pushed at `ec3fc845`; the separate first-enrollment preview remains DEV-only, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#首次强制-mfa-绑定隔离预览的开发验收证据). |
