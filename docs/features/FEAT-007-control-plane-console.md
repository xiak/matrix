...
| Gate | Evidence |
| --- | --- |
| Supported frontend runtime | Node 24.19.0; complete `build:embedded` plus type, lint, architecture and style gates passed. All 583 Vitest cases across 39 files and three static-normalization cases passed with the existing five-second per-test timeout unchanged: the full one-worker run passed 38 files and 580 cases while three existing policy-authoring cases crossed the timing boundary; the complete 121-case `AccessWorkspace` file then passed without a timeout override. |
| Theme and static-export boundary | 228 semantic contrast pairs passed across light, mixed and dark workspace/shell surfaces. Production preview was disabled; all 218 embedded files matched the normalized immutable export. |
...
| Personal MFA lifecycle preview | First enrollment starts only after a verified personal security-notification address and uses password-only step-up because no old factor exists. Correct confirmation of the new TOTP commits binding before one-time recovery material is shown; recovery acknowledgement is not the commit, and the material page has no cancel/reissue path. Bind, replace and legitimate removal enter a shared `REAUTHENTICATE` state that survives route remounts and disables replace/remove/recovery regeneration until normal preview logout/reset. Recovery input remains case-sensitive without trimming or uppercasing; cancelling recovery returns to normal login rather than the old challenge, and rebind never directly issues a normal session. Component, repository and `360 × 800` browser checks cover these boundaries without a dialog or horizontal overflow. Fixed IAM-009 interpretation and remaining LIVE integration are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人-mfa-生命周期-mock-的开发验收证据). |
