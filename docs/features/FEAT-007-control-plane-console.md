...
| Personal MFA lifecycle | Non-preview settings consume fixed first-TOTP enrollment state and commands; only first `APPLIED` can disclose provisioning in memory and confirmation requires exactly ten unique one-time codes before normal reauthentication. The login surface now also consumes the separately bounded S2b self-recovery candidate: one old code irreversibly replaces the old factor through a `RECOVERY/ENROLLMENT` challenge without creating a Session. Start-result uncertainty retains only the original non-secret request context for explicit inspection; `NOT_FOUND` never proves rollback, and confirmation uncertainty never replays the ten codes. The same provider is executable as a visibly labelled MOCK, with no LIVE failure fallback. Full ownership and remaining real-IAM acceptance are in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#受限自助-totp-恢复客户端的开发验收证据). |
| Fixed IAM login challenge client | Login consumes the strict IAM-009 `AUTHENTICATED` / `CHALLENGE_REQUIRED` union. `LOGIN/TOTP` remains sessionless until verification; exact `LOGIN/RECOVER` opens only the restricted recovery path, while a `RECOVERY` challenge is rejected from LoginResponse. Challenge-bound password replacement and recovery confirmation both clear restricted material and require a fresh login. Authentication generations discard late results after expiry, cancellation or a newer login. The DEV MFA entry runs the same state machine for direct success, password replacement and irreversible recovery, with visibly labelled inputs and no browser persistence. The latest recovery source and synchronized embed are pushed at `3c9a49fb`; full evidence and backend-candidate limitations are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#受限自助-totp-恢复客户端的开发验收证据). |
