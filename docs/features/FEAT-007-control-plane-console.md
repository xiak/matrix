...
Verified through 2026-09-25 against the current all-service navigation,
IAM policy-version authoring, group relationship refresh, isolated first-enrollment and account-rule previews.
...
| Supported frontend runtime | Node 23.11.1; the current source passed type, lint, architecture, styles, 764 Vitest cases across 44 files, three static-normalization cases, 41-page production export and 228-file byte-equivalent embed. `go test -p 2 ./...` and `go vet -p 2 ./...` also passed. Account-rule, activity-observation, authenticator-replacement and user-SSO previews remain isolated MOCK, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md). |
| IAM group relationship refresh | The live group detail retains the last verified member rows while a post-change authoritative read is pending, visibly marks the relationship as refreshing, and disables relation commands. A failed group read closes the old detail; a failed member read hides stale rows and offers local retry. Tests cover both transitions, rather than accepting a full-table skeleton flash as a normal fast response. Exact IAM behavior and browser limitations belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#用户组关系刷新片的开发验收证据). |
