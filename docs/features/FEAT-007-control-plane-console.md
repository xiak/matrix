...
| Supported frontend runtime | Node 23.11.1; the current source passed type, lint, architecture, styles, 756 Vitest cases across 44 files, three static-normalization cases, 41-page production export and 228-file byte-equivalent embed. `go test -p 2 ./...` and `go vet -p 2 ./...` also passed. Current source and synchronized embed are pushed at `e6ec141b`; account-rule, activity-observation, authenticator-replacement and user-SSO previews remain isolated MOCK, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md). |
