...
| Account security-settings preview | The settings workspace owns a separate semantic Account-security component instead of storing tenant rules inside the personal-factor flow. It now defaults to a read-only rule summary and enters edit explicitly, then performs exact-target review, scoped password-plus-TOTP step-up and preview save while keeping the fixed heading and scope mounted. A mapped conflict enters an inline stale-review state: the UI does not replay the write after refresh, discards the operation-bound proof and requires reload, review and fresh verification from the new baseline. The review preserves protected Root/platform-managed identities and tightening/relaxing session semantics. It remains preview-only, exposes no public IAM HTTP and never falls back to MOCK after a real failure. Preview settings retain only the owned Account MFA requirement and User SSO selection; stale password, session and generic sensitive-operation fields remain absent. Focus follows edit, review, step-up, conflict, reload, cancellation and completion without a dialog. The 124-case workspace suite, complete 603-case frontend run, 228-pair theme gate, 40-page export, 222-file embed equivalence, Go host and architecture/style gates passed. Default and `360 × 800` DEV checks produced no page overflow or console warning/error; the compact document and body both kept `clientWidth === scrollWidth === 360`. Design and IAM acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#账号级-mfa-要求-mock-的开发验收证据). |
