...
| IAM service-authorization preview | The preview Role directory now exposes a separate content-area service-authorization review without reclassifying ordinary service Roles. One isolated template identifies the service principal, purpose, predefined role name and the existing `MatrixDeliveryAccess` exact default-version snapshot, then reviews identity, cross-product permissions, explicit customer consent, short-lived session intent, non-expanding revisions and associated-resource revocation boundaries. The UI explicitly separates product onboarding, calling service, workload authentication and tenant consent; creates no domain model, command, Role, credential or success state; and leaves the final authorization button disabled because no fixed `ServiceRoleTemplate` tenant mutation or assume contract exists. The 42-file/659-case frontend run, three normalization cases, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. Default compact and `360 × 800` DEV checks reported no dialog, page overflow or warning/error; source and synchronized embed are pushed at `67d22770`. Contract and exclusion ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#服务授权-mock-的开发验收证据). |
