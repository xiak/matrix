...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 742 Vitest cases across 44 files passed with two workers and a 10-second per-test budget, together with three static-normalization cases; 41-page production export, 228-file byte-equivalent embed and `go test -p 2 ./...` / `go vet -p 2 ./...` also passed. The default five-second timing gate remains unclaimed. Current source and synchronized embed are committed at `49bc9722`; the account-rule, activity-observation and active authenticator replacement previews remain isolated MOCK, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人-mfa-生命周期的开发验收证据). |
