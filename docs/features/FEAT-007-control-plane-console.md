...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` and default `npm run check` passed: type, lint, architecture, style, 751 Vitest cases across 44 files, three static-normalization cases, 41-page production export and 228-file byte-equivalent embed. `go test -p 2 ./...` and `go vet -p 2 ./...` also passed. Current source and synchronized embed are pushed at `3b023711`; the account-rule, activity-observation, authenticator-replacement and user-SSO previews remain isolated MOCK, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md). |
