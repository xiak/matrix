...
| Access keys and settings | Key entry warns against Root long-lived keys and states that Secret material is only shown after an applied creation. Settings group account, login and security concerns without absorbing credential lifecycle. | Keep AccessKey management User-scoped and explicitly MOCK: one-time Secret acknowledgement, versioned state changes, disabled-before-delete and unknown-result recovery remain distinct. Reference key issuance, external workload verification, real security mutations and enrollment were not performed. |
...
Verified through 2026-09-24 against the current all-service navigation,
IAM policy-version authoring, isolated first-enrollment and account-rule previews.

| Gate | Evidence |
| --- | --- |
| IAM customer policy versions | LIVE detail retains lazy exact-version inspection and inline default/retirement review. Its CUSTOMER/TENANT version author offers JSON plus a catalog-driven visual path for one to 128 compatible exact Actions per statement, with paged choices and a selected-only filter; unrepresentable JSON remains untouched, and catalog denial is local to the visual path. Publication remains separate from default selection and authorization. The explicit MOCK editor and usage view remain separate. Source and synchronized embed are pushed at `8e2b0806`. The retained-MOCK mobile inspection does not establish LIVE write-browser acceptance or cross-session unknown-result recovery. Exact semantics and exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略版本读取与变更的开发验收证据). |
| IAM customer policy creation | The LIVE content-area author reuses the JSON and catalog-driven visual editor, requires an explicit Action and exact instance target by default, supports compatible multi-Action statements, then separates review, creation and attachment. The strict response parser accepts only the current Account's matching CUSTOMER/TENANT Policy and default document; uncertain transport/protocol outcomes retain the original request across IAM navigation, with byte-equivalent retry and explicit acknowledgement. The four-method MOCK author remains isolated; the same editor now has a separate, zero-write preview route. Source and synchronized embed are pushed at `8e2b0806`; real IAM browser and cross-login recovery remain unaccepted. Contract and UX evidence belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#live-自定义策略新建的开发验收证据). |
| Visual policy review | The shared LIVE/isolated-MOCK policy creation author now presents one current statement's effect, exact Actions, resource selectors and conditions before optional full JSON; multiple statements switch through a labelled selector instead of mounting the whole declaration at once. Free-form JSON keeps its full source visible because a partial summary could omit unknown fields. A `390px` DEV path from Action selection through review and JSON expansion had no document overflow; a fresh tab recorded no warning/error. The full 44-file/754-case frontend run, three export-normalization cases, type/lint/architecture/228-pair style checks, 41-page production export, 228-file embedded equivalence and Go test/vet passed. Source and embed are pushed at `3dddc532`; real-IAM browser acceptance remains open in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#live-自定义策略新建的开发验收证据). |
...
| Supported frontend runtime | Node 23.11.1; the current source passed type, lint, architecture, styles, 754 Vitest cases across 44 files, three static-normalization cases, 41-page production export and 228-file byte-equivalent embed. `go test -p 2 ./...` and `go vet -p 2 ./...` also passed. Current source and synchronized embed are pushed at `3dddc532`; account-rule, activity-observation, authenticator-replacement and user-SSO previews remain isolated MOCK, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md). |
...
| Responsive data hierarchy | The public native `Table` keeps horizontal scrolling as its default for dense comparison and selection surfaces and exposes an explicit stacked mobile layout for relationship, detail and compact workspace directories. The opt-in stack now activates at a `560px` table-container boundary rather than waiting for a phone-only width, so compact console canvases do not clip readable directory columns while dense comparison and selection tables retain the scrolling default. The stack retains the table/header semantics in the accessibility tree and supplies each visible value with its column label. Component coverage proves the shared contract and automatic workspace labels. Real DEV browser checks rendered the Role directory at `360px` and the IAM permission-capability directory at `539px` as readable labelled blocks; the latter reported document and body `clientWidth === scrollWidth === 539`, 18 labelled values and no dialog. User, Group, Policy and Role detail relationships use the same opt-in contract. The `539px` capability check was performed after a clean reload with no new warning/error; shared breakpoint source is pushed at `4e57ebdcec4c05fca0f223411460a1952d9c656a`. |
...
| IAM permission-capability directory | The policy workspace now lazily mounts a shared read-only capability directory after explicit tab selection, leaving initial IAM scene and policy loading unchanged. The LIVE adapter strictly consumes the fixed CAT-06 response and localizes 403, route mismatch, transport and protocol failures without MOCK fallback; the DEV repository supplies a visibly marked same-shape preview. Product actions open inline, not in a dialog, and entering/returning restores both focus and useful scroll context. The explicit preview now continues from one exact product declaration into a three-stage product engineering → IAM validation → trusted release review, while final publishing remains disabled because no draft, approval, publish API or Action exists. The entry is absent from LIVE, performs no repository write and does not model tenant registration or catalog visibility as a grant. The full 42-file/658-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 222-file embed equivalence and Go test/vet passed. `539px` and `360 × 800` DEV checks produced no page overflow, dialog or console warning/error. Source and synchronized embed are pushed at `5114dda3`; full contract and acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#权限能力目录片的开发验收证据). |
| IAM capability catalog display paging | The shared LIVE/MOCK product directory now displays only the current 10-row local page from the complete bounded snapshot, while product search resets to page 1 and detail return retains its prior page/focus. The Action pager remains independent and the footer explicitly avoids claiming backend paging. The 43-file/694-case frontend suite and three normalization cases, type/lint/architecture/228-pair style gates, 40-page export, 223-file embedded equivalence and Go test/vet passed. Browser inspection at `390 × 844` showed no document overflow or console warning/error; this remains DEV MOCK evidence, not real-IAM browser acceptance. Source and synchronized embed are at `b9ebf8ef`; exact IAM contract and UX acceptance are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#权限能力目录片的开发验收证据). |
| IAM service-authorization preview | The preview Role directory exposes a separate content-area service-authorization review without reclassifying ordinary service Roles. Its UX-only template identifies an illustrative target Account, service principal, purpose and predefined role name, and pins the existing `MatrixDeliveryAccess` v1 snapshot rather than following a later default. A v2 default remains visibly distinct; a missing pinned v1 blocks review instead of falling back. The first review step now states that a future PassRole/assumption path must jointly bind the actor, target Account/Role, actual workload and single service purpose; the current sample has no workload proof and therefore never represents an assumable credential. The remaining review separates cross-product permissions, explicit customer consent, short-lived session intent, non-expanding revisions and service-role deletion/associated-resource cleanup; authorization revocation and existing-session invalidation remain an unfixed backend contract. No domain model, command, Role, credential or success state is created, and final authorization remains disabled because no fixed `ServiceRoleTemplate` tenant mutation or assume contract exists. The 42-file/671-case frontend run, three normalization cases, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. DEV inspection confirmed the target Account and PassRole/workload boundary; source and synchronized embed are pushed at `c5ec1f945cdcd7af61941aafed8da4d7e68f839c`. Contract and exclusion ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#服务授权-mock-的开发验收证据). |
...
| IAM role/trust/session preview | Role metadata, policy attachment/removal, permission boundary, trust and session settings share content-area workflows instead of wide dialogs; destructive deletion alone retains a focused confirmation. The administrator session tab now contains only a searchable/filterable non-secret directory and one row overflow action for revocation. It no longer creates preview sessions or links to access simulation; member assumption remains in the separate self-service route. MOCK lifecycle changes schedule one wake-up at the next expiry boundary instead of polling and rerendering the directory every 15 seconds. The complete 42-file/671-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. A `390 × 844` DEV check kept the fixed Role heading, MOCK boundary, one search field, filters and mobile table readable without a dialog or page overflow. Source and synchronized embed are pushed at `c5ec1f945cdcd7af61941aafed8da4d7e68f839c`; the behavior remains explicit MOCK and fixed contract interpretation is owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#roletrust-与会话-mock-的开发验收证据). |
| IAM identity-provider preview | Identity-provider and federated-identity-mapping create/edit flows now replace only the content body of their stable directory/detail surfaces; destructive deletion remains a focused confirmation. The UI no longer calls an external-subject-to-role mapping a federated account, and continues to disclose that it performs no external discovery, invitation, directory sync or assertion validation. A shared directory workflow slot and responsive command focus handle return desktop users to the primary action and compact users to the visible page-action menu rather than a CSS-hidden button. The 130-case IAM workspace suite, complete 42-file/646-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 222-file embed equivalence and full Go test/vet passed. A `539 × 866` DEV check reported document/body width equality, zero dialogs, compact trigger focus and no warning/error. Source and synchronized embed are pushed at `fb08f614`; the behavior remains explicit MOCK and IAM contract/exclusion ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#身份提供商与联合身份映射-mock-的开发验收证据). |
| IAM member role self-service preview | The preview-only account menu opens a dedicated member self-service route instead of reusing the administrator Role/session directory. The scenario proves that an exact Assume-only member discovers only currently eligible roles and reviews source User, Account, Role version and duration without management reads. Executable preview paths keep generic authorization denial, generic state conflict and an unknown issuance outcome distinct: denial rediscoveries do not guess which authorization prerequisite changed; conflict never claims a stale Role by itself and only a known-fresh intent may rediscover through the member directory; unknown issuance retains and queries the original request, then revokes a credential-lost committed session before enabling a new request ID. The resulting MOCK identity exposes no credential and never changes the Header login; inline exit explains that Role logout does not issue or resurrect a USER credential. The full 590-case frontend run, 40-page export, 223-file embed equivalence, Go host and architecture/style gates passed. Default compact and `360 × 800` DEV checks reported zero dialogs, no page overflow and no final warning/error. Fixed contract interpretation and the intentionally absent LIVE adapter are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#成员自服务-role-承担-mock-的开发验收证据). |
| IAM Role and administrator-session management | The non-preview account repository strictly consumes Role list/read/create plus administrator RoleSession list/read/revoke from the fixed IAM-006 sources. Stable page chrome, heading, guidance, search and status controls render immediately; the session endpoint is not requested until its detail tab is selected, and only the session data region owns initial skeleton feedback. Role creation is a content-area, three-step workflow over exact same-account USER principals. It creates only Role metadata and the first immutable USER trust version: no policy attachment, permission boundary, password, long-lived key or temporary credential is implied. The adapter sends no Account selector and validates exact ownership, submitted metadata, ACTIVE state and initial trust; an unknown create result freezes the original payload and requestId and permits only an equivalent retry. Desktop commands share the title region and compact layouts expose the same actions through the common overflow menu. Exact session/source-user search, server lifecycle filtering and opaque continuation support empty windows without inventing totals or completion. The adapter validates exact ownership, ordering, bounds, versions, timestamps, canonical trust digest, 9/10 Role capability shapes, observed session lifecycle, source User and per-session revoke capability and never falls back to MOCK. Revocation stays inline. An uncertain revoke result is owned above keyed list/detail navigation and retains its original requestId across cancellation, tab remounts, returning to the Role directory and re-entering the same Role/Session; compare-and-set updates reject late callbacks, and Account/credential changes invalidate the intent. A later `REVOKED` read is not presented as proof that the uncertain command caused it, while authoritative `EXPIRED` is terminal and never offers revoke retry. Query and continuation requests use separate generations so a late old page cannot overwrite a newer filter. The 42-file/674-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. Desktop and `390 × 844` DEV inspection retained the MOCK Role surface and stable page commands without a dialog or page overflow. Source and synchronized embedded UI are pushed at `f0455570c3f5ae9f18bd266dffd5eb386e473c4e`; contract, exclusions and real-process acceptance remain owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#role-管理与管理员会话-live-客户端的开发验收证据). |
| IAM AccessKey lifecycle | The former account-wide key table remains replaced by a User selector and exact per-User key directory. The LIVE adapter now strictly consumes the fixed IAM-007 list/read/create/set-status/delete routes without sending a client-selected Account, and validates exact Account/User/key identity, resource versions, ordering and AccessKey-specific capability bindings. An `APPLIED` create may reveal the Secret once in memory; an `EQUAL_REPLAY` carrying a Secret fails closed and the UI shows only the non-secret completion result. Create, status and delete remain inline with stable request IDs and data-only loading; no real failure falls back to MOCK. The original explicit MOCK workspace remains available and preserves its executable unknown-result recovery scenarios. The complete 629-case frontend run, three normalization cases, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. A `390 × 844` DEV check verified the preserved MOCK directory/detail, zero dialogs, no page overflow and no warning/error; real IAM browser integration and product PEP support remain open. Source and synchronized embedded UI are pushed at `bd8b28d70d105f2994ab62bf5c2e0c83644d1975`; contract and acceptance ownership is [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#访问密钥生命周期的开发验收证据). |
| Personal security notification | Non-preview settings now consume the fixed IAM contact and verification routes through a strict current-user Provider. Contact and intent remain separate; exact ownership, request linkage, ten-minute timing and delivery observation are validated before rendering. `ACCEPTED` requires the fixed SMTP `250` observation and is explicitly not receipt/read evidence. Stable headings and cards paint immediately while only live data regions load; failures stay local and never fall back to MOCK. The explicit DEV experience retains the original no-IAM/no-SMTP preview. The complete 638-case frontend run, export/embed and Go gates passed; compact DEV preserved the labelled MOCK flow with no warning/error. Contract, exclusions and remaining real-process acceptance belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人安全通知地址的开发验收证据). |
...
| Isolated User SSO preview | The DEV-only configuration flow separates User SSO from Role SSO providers and reviews Account, protocol, mapping and sign-in impact inline before an enable/disable decision. Stable page chrome remains visible, and a `390 × 844` DEV check found no dialog, overflow or new warning/error. Complete frontend, export/embed and Go gates passed at pushed source `1df9ce06`; exact behavior and LIVE exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#用户-sso-隔离体验的开发验收证据). |
| User SSO review and compact account menu | Field-level preview-shape validation now stays in the editor and focuses the SAML/OIDC field before review; the SSO page no longer repeats its top title inside the card. The shared two-line account-menu rows use a four-pixel title/description gap rather than the previous two-pixel gap. A `390 × 844` real DEV check in light and mixed themes measured separate 53px menu rows and an unclipped panel, plus local field errors, no horizontal overflow, no dialog and no new warning/error. Current gates are summarized in the runtime row above; exact MOCK-only SSO boundaries belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#用户-sso-隔离体验的开发验收证据). |
...
| Account security settings | The separate MOCK rule remains read-only until the operator has bound personal TOTP and completed normal re-login; review and UNKNOWN recovery stay in the content area. The LIVE page adds a fixed, read-only current-account card backed only by the IAM read route: the rule body loads locally, 403 stays a local denial, and unavailable data never falls back to MOCK or exposes an edit action. A `390 × 844` DEV check kept the MOCK page at exactly 390px with no horizontal overflow. The 43-file/693-case frontend run, three normalization cases, type/lint/architecture/228-pair style gates, 40-page static export, 223-file embed match and full Go test/vet passed at pushed source `c62e7949`; this does not replace real IAM browser or release acceptance. Exact contract and exclusions are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#账号级-mfa-要求-mock-的开发验收证据). |
