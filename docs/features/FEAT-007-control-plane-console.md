...
IAM policy-version publication and isolated first-enrollment preview slices.

| Gate | Evidence |
| --- | --- |
| IAM customer policy versions | LIVE detail retains lazy exact-version inspection and inline default/retirement review; it now also authors and reviews a new CUSTOMER/TENANT JSON version inline. Publication remains separate from default selection and authorization. The explicit MOCK editor and usage view remain separate. Source and synchronized embed are pushed at `7def8761`. The complete 43-file/712-case Vitest suite passed with two workers and a 10-second per-test budget; three normalization cases, type/lint/architecture/228-pair style gates, 40-page production export, 223-file embed equivalence and `go test -p 2 ./...` / `go vet -p 2 ./...` also passed. The unchanged default five-second timing gate is not claimed as passed. The prior 390px retained-MOCK inspection does not establish LIVE write-browser acceptance or cross-session unknown-result recovery. Exact semantics and exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略版本读取与变更的开发验收证据). |
...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 715 Vitest cases across 43 files passed with two workers and a 10-second per-test budget, together with three static-normalization cases. The default five-second timing gate remains unclaimed. Current source and synchronized embed are pushed at `6c18a38b`; the new preview is DEV-only, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#首次强制-mfa-绑定隔离预览的开发验收证据). |
