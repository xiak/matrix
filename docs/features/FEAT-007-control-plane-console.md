...
| IAM member role self-service preview | The preview-only account menu now opens a dedicated member self-service route instead of reusing the administrator Role/session directory. The scenario proves that an exact Assume-only member can discover only currently eligible roles, review source User, Account, Role version and duration, and inspect safe unknown-outcome guidance without management reads. The resulting MOCK identity exposes no credential and never changes the Header login; inline exit explains that a real Role logout does not issue or resurrect a USER credential. The full 587-case frontend run, 40-page export, 223-file embed equivalence, Go host and architecture/style gates passed. Desktop and `360 × 800` DEV checks reported zero dialogs and no page overflow. Fixed contract interpretation and the intentionally absent LIVE adapter are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#成员自服务-role-承担-mock-的开发验收证据). |
