...
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 732 Vitest cases across 44 files passed with two workers and a 10-second per-test budget, together with three static-normalization cases; 41-page production export, 228-file embed equivalence and `go test -p 2 ./...` / `go vet -p 2 ./...` also passed. The default five-second timing gate remains unclaimed. Current source and synchronized embed are pushed at `8e2b0806`; the separate first-enrollment preview remains DEV-only, with exact IAM scope and exclusions in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#首次强制-mfa-绑定隔离预览的开发验收证据). |
