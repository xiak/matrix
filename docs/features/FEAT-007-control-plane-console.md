...
| IAM role/trust/session preview | Role trust selection, validation, before/after document review, preview assumption checks and single-session revocation now stay inside the relevant detail tab instead of opening large dialogs. The session directory uses one row overflow menu rather than adjacent diagnostic/revoke buttons, states that lifecycle is not current authority, and keeps preview creation explicitly non-credentialing. The complete 121-case workspace suite passed; all five unrelated policy cases that crossed the unchanged five-second boundary in the full 583-case run passed together when isolated. Production export/equivalence and the Go host passed. Default `539px` and `360 × 800` DEV checks reported no dialog, warning/error or page overflow, with deterministic focus restoration. Fixed Role/STS contract interpretation and remaining LIVE integration are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#roletrust-与会话-mock-的开发验收证据). |
