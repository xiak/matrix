...
| Gate | Evidence |
| --- | --- |
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 603 Vitest cases across 40 files and three static-normalization cases passed with the existing five-second per-test timeout unchanged. |
...
| Responsive data hierarchy | The public native `Table` keeps horizontal scrolling as its default for dense comparison and selection surfaces and exposes an explicit stacked mobile layout for relationship, detail and compact workspace directories. The stack retains the table/header semantics in the accessibility tree and supplies each visible value with its column label. Component coverage proves the shared contract and automatic workspace labels. A `360px` real DEV browser check rendered the Role directory as readable labelled blocks with body, region and table `clientWidth === scrollWidth` (`360px` page, `324px` table); User, Group, Policy and Role detail relationships use the same opt-in contract while high-density selection and account-session directories retain the scrolling variant. |
...
| Forced first-login password replacement | The initial password establishes only a restricted password-change session. A successful replacement invalidates that credential, clears the client identity and returns to normal sign-in with an explicit success notice; it never upgrades the restricted session in place or navigates directly to the requested console page. Failures remain inside the restricted scene. Provider, renderer and preview-repository cases prove the old opaque credential receives 401 and a later login receives a different credential. The complete 603-case frontend run, export/embed and Go host gates passed; real DEV one-click login, console navigation and clean reload emitted no new warning/error. Backend password storage, failure counting, conflicts and authoritative re-login remain owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#首次登录强制改密-mock-的开发验收证据). |
| Account security-settings preview | The settings workspace now owns a separate semantic Account-security component instead of storing tenant rules inside the personal-factor flow. It follows edit, exact-target review, scoped password-plus-TOTP step-up and preview-save stages; the review preserves protected Root/platform-managed identities and the tightening/relaxing session semantics. It is explicitly preview-only, exposes no public IAM HTTP and never falls back to MOCK after a real failure. Preview settings now retain only the owned Account MFA requirement and User SSO selection; stale password-length, expiry, reuse, Account-session and generic sensitive-operation fields were removed from the domain, repository and report rather than merely hidden. The security overview consequently renders five evidence-based checks and no phantom sensitive-operation status. Focus follows review, step-up, return, cancellation and completion without a dialog. The 121-case workspace suite, 53-case preview-repository suite, embedded build/equivalence, Go host and architecture/style gates passed. Default and `360 × 800` DEV checks produced no page overflow or console warning/error; the compact document and body both kept `clientWidth === scrollWidth === 360`. Design and IAM acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#账号级-mfa-要求-mock-的开发验收证据). |
