...
| Gate | Evidence |
| --- | --- |
| Supported frontend runtime | Node 24.19.0; complete `build:embedded` plus type, lint, architecture, style and test gates passed: 583 Vitest cases across 39 files and three static-normalization cases. The complete Vitest suite used one local worker with the existing five-second per-test timeout unchanged; two parallel attempts exposed timing-only failures in existing policy-authoring cases, and each affected case passed alone. |
...
| Personal security-notification preview | The settings workspace now owns a standalone, explicitly marked MOCK block for the first personal security-notification address. Address/current-password, eight-digit code and confirmed summary render inline without a dialog; state is page-local, sends no IAM request or real mail, and labels `ACCEPTED` as channel acceptance rather than delivery or reading. Replacement, deletion and administrator override stay absent. Focus moves to the mounted workflow and confirmed summary, then returns to the stable trigger on cancellation. Desktop and `360 × 800` DEV checks produced no page overflow or console warning/error; the compact document and body both kept `clientWidth === scrollWidth === 360`. Design and IAM acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人安全通知地址-mock-的开发验收证据). |
