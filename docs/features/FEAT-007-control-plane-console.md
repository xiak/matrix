...
| IAM AccessKey lifecycle preview | The former account-wide key table is replaced by a User selector and exact per-User key directory. Inline create, status and delete flows show the User revision, key resource version and request ID without a dialog; an applied create reveals the secret once and removes it from page state after acknowledgement. The executable unknown-outcome path stores only the owner and original request ID, not an unknown key ID; it survives route remounts. The owner/requestId result mapping is immutable: exact create replay cannot create a second key or reissue a Secret. `NOT_FOUND` and temporarily unavailable lookups both keep UNKNOWN and the global create lock; only a verified committed result reveals the non-secret key ID. A committed key whose secret was lost must be disabled and deleted before a new request ID is enabled. Creation and re-enable require an active User without forced password change, while defensive read/disable/delete stay distinct and LIVE authorization remains an AccessKey-specific capability. The page does not invent last-used evidence, keeps login/session lifecycle separate and teaches create-second, externally verify, disable-old, delete-old rotation. The full 602-case frontend run, 40-page export, 222-file embed equivalence, full Go test/vet and architecture/style gates passed. Real DEV exercised all three lookup results and a clean reload with no new warning/error. Fixed IAM-007 interpretation and remaining LIVE integration are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#访问密钥生命周期-mock-的开发验收证据). |
| Personal security-notification preview | The settings workspace now owns a standalone, explicitly marked MOCK block for the first personal security-notification address. Address/current-password, eight-digit code and confirmed summary render inline without a dialog; state is page-local and sends no IAM request or real mail. Contact state, verification intent and delivery observation are separate: a new intent is `PENDING` while the contact remains `NONE`, and the MOCK explicitly has no SMTP acceptance evidence. The vocabulary still distinguishes a future authoritative `ACCEPTED` observation from delivery or reading. Replacement, deletion and administrator override stay absent. Focus moves to the mounted workflow and confirmed summary, then returns to the stable trigger on cancellation. Desktop and `360 × 800` DEV checks produced no page overflow or console warning/error; the compact document and body both kept `clientWidth === scrollWidth === 360`. Design and IAM acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人安全通知地址-mock-的开发验收证据). |
| Personal MFA lifecycle preview | First enrollment starts only after a verified personal security-notification address and uses password-only step-up because no old factor exists. Correct confirmation of the new TOTP commits binding before one-time recovery material is shown; recovery acknowledgement is not the commit, and the material page has no cancel/reissue path. Bind, replace and legitimate removal enter a shared `REAUTHENTICATE` state. Normal logout preserves committed factor/removal/recovery history; only the explicit demo reset returns the repository to its initial state. Confirmation or legitimate removal invalidates the old browser credential in the same repository transition; the recovery-material page is then a local result surface and leaving it clears the local session. Recovery requires the current password plus one exact, unused, case-sensitive recovery item and consumes it when issuing one opaque restricted challenge. Persistent `rebind-required` never grants setup by itself: cancelling or losing that challenge requires the password and another unused item. New TOTP confirmation commits the replacement and invalidates the old bearer before recovery codes; a fresh password-plus-TOTP login issues a new opaque credential and the old bearer remains rejected. The full 602-case frontend run, repository/component coverage and real DEV checks cover ordinary logout, wrong-password preservation, consumed-code rejection, interrupted recovery, replacement and fresh login; a clean reload emitted no new warning/error. Fixed IAM-009 interpretation and remaining LIVE integration are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人-mfa-生命周期-mock-的开发验收证据). |
