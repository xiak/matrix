...
| Account security-settings preview | The separate MOCK Account rule stays read-only until the operator has a personal TOTP factor and completes normal re-login. Without a factor, an inline action focuses the personal-security section; an old Session with pending reauthentication cannot edit. The rule review remains content-area based, with distinct denial, conflict and original-intent UNKNOWN states and no LIVE fallback. A `390 × 844` DEV check kept document/body width at 390px, focused the target heading and emitted no warning/error. Source and synchronized embed are pushed at `3b954402`; future independent read/update capability and real-process acceptance remain owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#账号级-mfa-要求-mock-的开发验收证据). |
