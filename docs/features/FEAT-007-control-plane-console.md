...
| Gate | Evidence |
| --- | --- |
| Supported frontend runtime | Node 23.11.1; complete `build:embedded` plus type, lint, architecture and style gates passed. All 602 Vitest cases across 40 files and three static-normalization cases passed with the existing five-second per-test timeout unchanged. |
| Theme and static-export boundary | 228 semantic contrast pairs passed across light, mixed and dark workspace/shell surfaces. Production preview was disabled; all 222 embedded files matched the normalized immutable export. |
...
| IAM service-authorization preview | The preview Role directory now exposes a separate content-area service-authorization review without reclassifying ordinary service Roles. Its UX-only template identifies an illustrative service principal, purpose and predefined role name, and pins the existing `MatrixDeliveryAccess` v1 snapshot rather than following a later default. A v2 default remains visibly distinct; a missing pinned v1 blocks review instead of falling back. The review separates identity, cross-product permissions, explicit customer consent, short-lived session intent, non-expanding revisions and service-role deletion/associated-resource cleanup; authorization revocation and existing-session invalidation remain an unfixed backend contract. No domain model, command, Role, credential or success state is created, and final authorization remains disabled because no fixed `ServiceRoleTemplate` tenant mutation or assume contract exists. The shared command handle restores the exact Service authorization button on desktop and the visible page-action trigger in compact mode. The 42-file/662-case frontend run, three normalization cases, type/lint/architecture/228-pair style gates, 40-page export, 223-file embed equivalence and full Go test/vet passed. Compact and `1600 × 900` DEV checks confirmed the responsive focus targets with no warning/error; source and synchronized embed are pushed at `846d23b3`. Contract and exclusion ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#服务授权-mock-的开发验收证据). |
...
| IAM role/trust/session preview | Role metadata, policy attachment/removal, permission boundary, trust and new-session settings now share content-area workflows instead of wide dialogs; destructive deletion alone retains a focused confirmation. The common inline form owns heading/error focus, one in-flight submit, disabled dismissal while pending and retained inputs after failure. Selection-sensitive focus returns to the original trigger, or to the still-valid attach action after detaching the final policy. Trust review, preview assumption checks and single-session revocation remain inline; the session directory uses one row overflow menu and never claims that lifecycle is current authority or that preview creation issues credentials. The 129-case workspace suite, complete 42-file/645-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 222-file embed equivalence and full Go test/vet passed. A `539 × 866` DEV check reported document/body width equality, zero dialogs, current-workflow focus and no warning/error. Source and synchronized embed are pushed at `6066bf2be9eed396341a83e6461aa6b38658ea33`; the behavior remains explicit MOCK and fixed contract interpretation is owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#roletrust-与会话-mock-的开发验收证据). |
...
| IAM Role read-only management | The non-preview account repository now exposes strict Role list/read operations from the fixed IAM-006 source while the explicit DEV experience keeps its richer synthetic Role/trust/session preview independent. Stable page chrome, heading, guidance, search and status controls render immediately; only the directory or detail data region loads. The directory uses the opaque management cursor without inventing a total and opts into labelled mobile stacking. Role details remain inline and separate metadata, policy attachments, exact same-account User trust and response-scoped capabilities; they never open a dialog or reinterpret preview service/provider/cross-account examples as LIVE support. The adapter validates exact ownership, ordering, bounds, versions, timestamps, canonical trust digest and capability-resource bindings, sends no Account selector and never falls back to MOCK. The 42-file/645-case frontend run, three normalization checks, type/lint/architecture/228-pair style gates, 40-page export, 222-file embed equivalence and full Go test/vet passed. Source and synchronized embedded UI are pushed at `d4bff0d368996833a4bde1c5c44bfad29fe03e4c`; contract, exclusions and real-process acceptance remain owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#role-管理只读-live-客户端的开发验收证据). |
