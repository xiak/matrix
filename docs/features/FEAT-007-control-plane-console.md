...
| Account security-settings preview | The settings workspace owns a separate semantic Account-security component instead of storing tenant rules inside the personal-factor flow. It defaults to a read-only rule summary, then uses explicit edit, exact-target review and scoped password-plus-TOTP step-up. Account-rule and User-SSO saves are separate preview commands; readable state does not imply update capability. Update denial and version conflict both discard operation-bound proof and secrets without replay. A lost save response persists the original intent as `UNKNOWN` across route remounts, suppresses success feedback and blocks a second write; `NOT_FOUND` and unavailable inspection stay unknown, while only authoritative applied/rejected results unlock the flow. The review preserves protected Root/platform-managed identities and tightening/relaxing session semantics. It remains preview-only, exposes no public IAM HTTP and never falls back to MOCK after a real failure. The 190-case targeted workspace/repository run, complete 607-case frontend run, 228-pair theme gate, 40-page export, 222-file embed equivalence, full Go test/vet and architecture/style gates passed. A `539px` DEV journey proved route-persistent locking, no dialog or page overflow and `clientWidth === scrollWidth === 539`; source and synchronized embedded UI are pushed at `b27adbdc6701575e19ac02e1550fba8264860422`. Design and IAM acceptance ownership remains in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#账号级-mfa-要求-mock-的开发验收证据). |
