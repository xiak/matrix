...
| Personal MFA lifecycle | Non-preview settings now consume fixed first-TOTP enrollment state and commands. Only first `APPLIED` returns provisioning in the current memory flow; `EQUAL_REPLAY` cannot render a seed, URI or confirmation field. Confirmation accepts exactly ten unique one-time recovery items, clears the revoked session, shows them once from provider memory and then requires a normal login. `BOUND` and `RECOVERY_REQUIRED` remain read-only because replacement/removal/recovery have no fixed UI contract; the explicit DEV experience retains the richer MOCK recovery journey without becoming a LIVE fallback. Full contract and remaining real-IAM browser acceptance belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#本人-mfa-生命周期的开发验收证据). |
| Fixed IAM login challenge client | Login now consumes the fixed IAM-009 `AUTHENTICATED` / `CHALLENGE_REQUIRED` union instead of treating a password response as an unconditional session. A TOTP challenge remains sessionless and memory-only until verification succeeds. Verification may move to a distinct challenge-bound password replacement, whose success clears all authentication material and requires a fresh login; it never reuses the ordinary authenticated `/auth/password` flow or navigates directly into the console. Authentication generations also discard a late success or failure after expiry, cancellation or a newer login, so an old challenge cannot overwrite the newer principal, phase or error. The DEV MFA entry runs the same provider and repository boundary with visibly labelled codes for direct success and password replacement, while recovery remains an isolated preview pending a separately fixed backend contract. The 622-case full frontend run, three export-normalization tests, type/lint/architecture/228-pair style checks, 40-page production export and 223-file embed equivalence passed. Desktop and `390 × 844` mixed/dark browser checks showed no overflow or final warning/error. Source and synchronized embedded UI are pushed at `1e1e021fdde9cbfc8078b2e3ca42f026ea7faf3d`; IAM ownership and exclusions remain in [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#登录挑战固定契约的开发验收证据). |
