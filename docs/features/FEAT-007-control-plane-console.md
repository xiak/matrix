...
| Gate | Evidence |
| --- | --- |
| IAM customer policy-version inspection | The LIVE detail exposes a lazy version tab only for active CUSTOMER/TENANT policies. A bounded, sorted five-version directory and an independently authorized exact non-default read stay in the content area with regional loading/error states and focus return; System and installation policies cannot use this route. The explicit MOCK version editor and usage view remain separate. Source and synchronized embed are pushed at `a4f2a0b7`. The 43-file/702-case Vitest suite passed with two workers and a 10-second per-test budget, together with three normalization cases, type/lint/architecture/228-pair style gates, 40-page production export, 223-file embed equivalence and `go test -p 2 ./...` / `go vet -p 2 ./...`. This does not clear the previously observed unchanged five-second timing gate, does not test a real IAM browser and does not implement version writes. Exact contract and exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略默认版本只读详情的开发验收证据). |
| IAM policy default-detail read | The LIVE policy directory opens an exact TENANT policy in the content area and reads only its current default immutable version on demand. Stable heading and metadata render first; only the document region loads or reports a local denial. Platform policy metadata never triggers a tenant detail request, and v2 authored Action families stay visually distinct from their frozen exact expansion. The explicit MOCK policy authoring/version/usage experience remains separate. Source and synchronized embed are pushed at `2c3dc421`. Type/lint/architecture/228-pair style gates, 40-page production export, 223-file embed equivalence, three normalization cases, `go test -p 2 ./...` and `go vet -p 2 ./...` passed. The 43-file/699-case Vitest suite passed with two workers and a 10-second per-test budget; the unchanged default five-second `npm run check` first failed five, then single-file reruns failed one different long MOCK wizard case, so the default timing gate is not claimed as passed. No real-IAM browser acceptance was run. IAM contract and exclusions belong to [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#策略默认版本只读详情的开发验收证据). |
