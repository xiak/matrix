...
| IAM AccessKey lifecycle preview | The former account-wide key table is replaced by a User selector and exact per-User key directory. Inline create, status and delete flows show the User revision, key resource version and request ID without a dialog; an applied create reveals the secret once and removes it from page state after acknowledgement. The executable unknown-outcome path retains and queries the original request, then requires disable/delete of a committed key whose secret was lost before a new request ID is enabled. The page does not invent last-used evidence, keeps login/session lifecycle separate and teaches create-second, externally verify, disable-old, delete-old rotation. The full 591-case frontend run, 40-page export, 223-file embed equivalence, Go host and architecture/style gates passed. At `360 × 800`, document `clientWidth === scrollWidth === 360` and no dialog was present. Fixed IAM-007 interpretation and remaining LIVE integration are owned by [FEAT-IAM-010](../../IAM/FEAT-IAM-010-console.md#访问密钥生命周期-mock-的开发验收证据). |
