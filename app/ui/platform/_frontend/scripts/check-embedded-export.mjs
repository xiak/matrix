...
if (
  ...
  basename(deliveryUnit) !== "platform" ||
...
if (JSON.stringify(exported) !== JSON.stringify(embedded)) {
  throw new Error("embedded platform assets drifted; run npm run build:embedded");
}

process.stdout.write(`embedded platform export matches ${exported.length} generated files\n`);
