...
import { Button, EmptyState, PageSkeleton, useLeaveConfirmation } from "@ui/xiak";
...
export function ProductGate({ id, children }: { id: ProductID; children: ReactNode }) {
  ...
  return <div className={styles.stack}>{!canMutate ? <div className={styles.feedback} role="status"><span><strong>{t("readOnly")}</strong><br />{t("readOnlyHint")}</span><Button variant="ghost" onClick={() => leave(inventory.refresh)}>{t("recheckReadiness")}</Button></div> : null}{children}</div>;
}
