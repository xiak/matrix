...
import { Button, Checkbox, Dialog, FormField, Input, RadioGroup, SearchInput, Select, Table } from "../index";
...
describe("shared themed controls", () => {
  ...
  it("styles a real link as a button without nesting interactive elements", async () => {
    ...
    const user = userEvent.setup();
    render(<Button asChild><a href="/destination" onClick={onClick}>Dashboard</a></Button>);
  ...
  it("keeps tables native and scrollable, with status announcements separate from data", () => {
    render(<><Table aria-label="Resources"><thead><tr><th scope="col">Name</th></tr></thead><tbody><tr><td>PostgreSQL</td></tr></tbody></Table></>);
