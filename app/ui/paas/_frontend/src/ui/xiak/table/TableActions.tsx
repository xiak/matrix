...
export function TableActions({ label, disabled, hint, selectionLabel, clearLabel, onClear, actions, triggerRef }: {
  ...
  return <div className={styles.actions}>
    <Menu.Root modal={false}>
      <Menu.Trigger asChild><Button ref={triggerRef} size="small" variant="secondary" disabled={disabled} title={hint}>{label}<ChevronDown aria-hidden="true" /></Button></Menu.Trigger>
      <Menu.Portal><Menu.Content aria-label={label} align="end" collisionPadding={12} sideOffset={6} className={menuStyles.popup} onCloseAutoFocus={(event) => { if (chosen.current) event.preventDefault(); chosen.current = false; }}>
        {actions.map((action, index) => <Menu.Group key={action.id}>
          ...
          <Menu.Item textValue={action.label} className={menuStyles.item} data-danger={action.danger || undefined} disabled={Boolean(action.disabledReason)} aria-describedby={action.disabledReason ? `${reasonId}-${reasons.indexOf(action.disabledReason)}` : undefined} onSelect={() => { chosen.current = true; action.onSelect(); }}>
