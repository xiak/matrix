...
import { userRoles, type Account, type AccountIdentity, type AccountPrincipal, type AccountUser, type DirectoryPage, type UserRole, type UserRoleBinding } from "../domain/accounts";
import type {
...
function parseUserRole(value: unknown): UserRole {
  if (!userRoles.includes(value as UserRole)) throw new Error("INVALID_IAM_RESPONSE");
...
function parseRoleBinding(value: unknown): UserRoleBinding {
  ...
  return { id: accountText(wire.id), principalId: accountText(wire.principalId), organizationId: accountText(wire.organizationId), role: parseUserRole(wire.role) };
}
...
function parseAccountIdentity(value: unknown): AccountIdentity {
  ...
  const roles = wire.roles.map(parseUserRole);
