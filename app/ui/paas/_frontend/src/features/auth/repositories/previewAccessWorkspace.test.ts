...
import { beginPreviewPersonalMfaRecovery, cancelPreviewPersonalMfaRecovery, nextPreviewPersonalMfaRecoveryCode, preparePreviewPersonalMfaDemo, previewAccountRepository, previewCredential, previewIamRepository, previewPersonalMfaSnapshot, resetPreviewEnvironment } from "./previewIamRepository";
...
describe("access workspace preview invariants", () => {
  ...
  it("keeps the old factor and codes valid until a session-bound replacement commits", async () => {
    const repository = createPreviewAccessWorkspace("org-xiak", () => ["principal-lin"], "admin");
    repository.transact((source) => ({ workspace: { ...source, personalMfa: { factorState: "bound", reauthenticationRequired: false, recoveryState: "idle" } } }));
    const oldCodes = repository.recoveryCodes();
    const proofStartedAt = new Date(Date.now() - 10_000).toISOString();
    const started = await repository.execute("session-a", { kind: "begin-personal-mfa-replacement", requestId: "replace-one", proofStartedAt });
    expect(started.workspace.personalMfa).toMatchObject({ factorState: "bound", reauthenticationRequired: false, pendingReplacement: { requestId: "replace-one", accountRuleVersion: 1 } });
    expect(started.workspace.personalMfa.pendingReplacement?.expiresAt).toBe(new Date(Date.parse(proofStartedAt) + 120_000).toISOString());
    expect(repository.recoveryCodes()).toEqual(oldCodes);
    await expect(repository.execute("session-b", { kind: "confirm-personal-mfa-replacement", requestId: "replace-one" })).rejects.toMatchObject({ code: "invalid" });
    await expect(repository.execute("session-a", { kind: "remove-personal-mfa" })).rejects.toMatchObject({ code: "invalid" });
    const completed = await repository.execute("session-a", { kind: "confirm-personal-mfa-replacement", requestId: "replace-one" });
    expect(completed.workspace.personalMfa).toEqual({ factorState: "bound", reauthenticationRequired: true, recoveryState: "idle", demoCode: "731942" });
    expect(completed.recoveryCodes).toHaveLength(10);
    expect(new Set(completed.recoveryCodes).size).toBe(10);
    expect(repository.recoveryCodes()).toEqual(completed.recoveryCodes);
  ...
  it("issues exactly one new preview recovery batch after an approved regeneration", async () => {
    ...
    repository.transact((source) => ({ workspace: { ...source, personalMfa: { factorState: "bound", reauthenticationRequired: false, recoveryState: "idle" } } }));
    const oldCodes = repository.recoveryCodes();
    const result = await repository.execute("session-a", { kind: "regenerate-personal-recovery-codes" });
    expect(result.recoveryCodes).toHaveLength(10);
    expect(result.recoveryCodes).toEqual(repository.recoveryCodes());
    expect(result.recoveryCodes?.some((code) => oldCodes.includes(code))).toBe(false);
    expect(result.workspace.personalMfa).toEqual({ factorState: "bound", reauthenticationRequired: false, recoveryState: "idle" });
  });
  it("cancels a replacement without changing the old factor and rejects expired or stale intents", async () => {
    const repository = createPreviewAccessWorkspace("org-xiak", () => ["principal-lin"], "admin");
    repository.transact((source) => ({ workspace: { ...source, personalMfa: { factorState: "bound", reauthenticationRequired: false, recoveryState: "idle" } } }));
    const oldCodes = repository.recoveryCodes();
    await repository.execute("session-a", { kind: "begin-personal-mfa-replacement", requestId: "replace-one", proofStartedAt: new Date().toISOString() });
    await repository.execute("session-b", { kind: "cancel-personal-mfa-replacement", requestId: "replace-one" });
    expect((await repository.read("session-b")).personalMfa).toEqual({ factorState: "bound", reauthenticationRequired: false, recoveryState: "idle" });
    expect(repository.recoveryCodes()).toEqual(oldCodes);
    const started = await repository.execute("session-b", { kind: "begin-personal-mfa-replacement", requestId: "replace-two", proofStartedAt: new Date().toISOString() });
    const expired = structuredClone(started.workspace);
    expired.personalMfa.pendingReplacement!.expiresAt = "2026-09-09T02:00:00Z";
    expect(() => applyAccessWorkspaceCommand(expired, { kind: "confirm-personal-mfa-replacement", requestId: "replace-two" }, { ...context, sameReplacementSession: true })).toThrow();
    const stale = structuredClone(started.workspace);
    stale.settings.accountRuleVersion += 1;
    expect(() => applyAccessWorkspaceCommand(stale, { kind: "confirm-personal-mfa-replacement", requestId: "replace-two" }, { ...context, sameReplacementSession: true })).toThrow();
    const expiredProof = { kind: "begin-personal-mfa-replacement" as const, requestId: "late", proofStartedAt: "2026-09-09T01:58:00Z" };
    const bound = initialAccessWorkspace("org-xiak");
    bound.personalMfa = { factorState: "bound", reauthenticationRequired: false, recoveryState: "idle" };
    expect(() => applyAccessWorkspaceCommand(bound, expiredProof, context)).toThrow();
  });
  it("rejects the old demonstration factor and recovery code after replacement", async () => {
    resetPreviewEnvironment();
    preparePreviewPersonalMfaDemo();
    const oldCode = nextPreviewPersonalMfaRecoveryCode();
    await previewAccountRepository.workspace!.execute(previewCredential, { kind: "begin-personal-mfa-replacement", requestId: "replace-login", proofStartedAt: new Date().toISOString() });
    expect(nextPreviewPersonalMfaRecoveryCode()).toBe(oldCode);
    const confirmed = await previewAccountRepository.workspace!.execute(previewCredential, { kind: "confirm-personal-mfa-replacement", requestId: "replace-login" });
    expect(confirmed.recoveryCodes).toHaveLength(10);
    expect(nextPreviewPersonalMfaRecoveryCode()).toBe(confirmed.recoveryCodes?.[0]);
    expect(await beginPreviewPersonalMfaRecovery("demo-password", oldCode!)).toBeNull();
    const challenge = await previewIamRepository.login({ loginName: "preview-admin", password: "experience-only" });
    if (challenge.outcome !== "CHALLENGE_REQUIRED") throw new Error("expected challenge");
    const input = { challengeId: challenge.challenge.id, challengeCredential: challenge.challengeCredential };
    await expect(previewIamRepository.authenticationChallenges!.verify({ ...input, code: "624810" })).rejects.toMatchObject({ status: 401 });
    await expect(previewIamRepository.authenticationChallenges!.verify({ ...input, code: "731942" })).resolves.toMatchObject({ outcome: "AUTHENTICATED" });
    resetPreviewEnvironment();
  });
  ...
  it("revokes the old preview bearer at binding confirmation and never revives it after a fresh login", async () => {
    ...
    const challenge = await previewIamRepository.login({ loginName: "preview-admin", password: "experience-only" });
    if (challenge.outcome !== "CHALLENGE_REQUIRED") throw new Error("expected preview challenge");
    const freshSession = await previewIamRepository.authenticationChallenges!.verify({ challengeId: challenge.challenge.id, challengeCredential: challenge.challengeCredential, code: "624810" });
    ...
    expect(previewPersonalMfaSnapshot().reauthenticationRequired).toBe(false);
    await expect(previewAccountRepository.currentIdentity(oldSession.credential)).rejects.toMatchObject({ status: 401 });
  });
  it("queries an unknown account-rule intent only after TOTP sign-in without expiring the new session", async () => {
    resetPreviewEnvironment();
    preparePreviewPersonalMfaDemo();
    const firstChallenge = await previewIamRepository.login({ loginName: "preview-admin", password: "experience-only" });
    if (firstChallenge.outcome !== "CHALLENGE_REQUIRED") throw new Error("expected preview challenge");
    const firstSession = await previewIamRepository.authenticationChallenges!.verify({ challengeId: firstChallenge.challenge.id, challengeCredential: firstChallenge.challengeCredential, code: "624810" });
    if (firstSession.outcome !== "AUTHENTICATED") throw new Error("expected preview session");
    const unknown = await previewAccountRepository.workspace!.execute(firstSession.credential, { kind: "save-account-rule", requestId: "account-rule-lost", expectedRuleVersion: 1, expectedLoginProtection: false, loginProtection: true, responseMode: "response-lost" });
    expect(unknown.workspace.pendingAccountRuleChange?.status).toBe("UNKNOWN");
    await expect(previewAccountRepository.workspace!.execute(firstSession.credential, { kind: "inspect-account-rule-change", requestId: "account-rule-lost", resultMode: "found-applied" })).rejects.toMatchObject({ code: "invalid" });
    const secondChallenge = await previewIamRepository.login({ loginName: "preview-admin", password: "experience-only" });
    if (secondChallenge.outcome !== "CHALLENGE_REQUIRED") throw new Error("expected preview challenge");
    const secondSession = await previewIamRepository.authenticationChallenges!.verify({ challengeId: secondChallenge.challenge.id, challengeCredential: secondChallenge.challengeCredential, code: "624810" });
    if (secondSession.outcome !== "AUTHENTICATED") throw new Error("expected preview session");
    const observed = await previewAccountRepository.workspace!.execute(secondSession.credential, { kind: "inspect-account-rule-change", requestId: "account-rule-lost", resultMode: "found-applied" });
    expect(observed.workspace.settings).toMatchObject({ loginProtection: true, accountRuleVersion: 2 });
    expect(observed.workspace.personalMfa.reauthenticationRequired).toBe(false);
    expect(await previewAccountRepository.currentIdentity(secondSession.credential)).toBeTruthy();
  });
  ...
  it("locks an unknown account-rule intent until its original result is definitive", () => {
    ...
    expect(() => applyAccessWorkspaceCommand(initial, { kind: "save-account-rule", requestId: "no-change", expectedRuleVersion: 1, expectedLoginProtection: false, loginProtection: false, responseMode: "success" }, context)).toThrow("invalid");
    ...
    expect(journaled.personalMfa.reauthenticationRequired).toBe(true);
    ...
    expect(unknown.personalMfa.reauthenticationRequired).toBe(true);
    ...
    expect(() => applyAccessWorkspaceCommand(unknown, { kind: "inspect-account-rule-change", requestId: "account-rule-1", resultMode: "found-applied" }, context)).toThrow("invalid");
    const reauthenticated = applyAccessWorkspaceCommand(unknown, { kind: "complete-personal-mfa-reauthentication" }, context);
    expect(reauthenticated.pendingAccountRuleChange).toEqual(unknown.pendingAccountRuleChange);
    expect(() => applyAccessWorkspaceCommand(reauthenticated, { kind: "inspect-account-rule-change", requestId: "account-rule-1", resultMode: "not-found" }, context)).toThrow("accountRuleResultNotFound");
    expect(() => applyAccessWorkspaceCommand(reauthenticated, { kind: "inspect-account-rule-change", requestId: "account-rule-1", resultMode: "unavailable" }, context)).toThrow("accountRuleResultUnavailable");
    const applied = applyAccessWorkspaceCommand(reauthenticated, { kind: "inspect-account-rule-change", requestId: "account-rule-1", resultMode: "found-applied" }, context);
    expect(applied.settings.loginProtection).toBe(true);
    expect(applied.settings.accountRuleVersion).toBe(2);
    expect(applied.personalMfa.reauthenticationRequired).toBe(false);
    ...
    const reauthenticatedAgain = applyAccessWorkspaceCommand(another, { kind: "complete-personal-mfa-reauthentication" }, context);
    const rejected = applyAccessWorkspaceCommand(reauthenticatedAgain, { kind: "inspect-account-rule-change", requestId: "account-rule-3", resultMode: "found-rejected" }, context);
