...
type FlowError = "load" | "contactStart" | "contactConfirm" | "enrollmentStart" | "enrollmentConfirm" | "cancel" | null;
...
export function LivePersonalSecuritySettings() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextContact, nextFactor] = await Promise.all([client.notificationContact(), client.authenticatorState()]);
  ...
  async function startEnrollment(event: FormEvent<HTMLFormElement>) {
    ...
    } catch { setError("enrollmentStart"); }
    finally { setBusy(false); }
  }
      ...
      <Card>
        ...
        <Card.Body className={styles.cardBody}>
          ...
          {loadState === "ready" && enrollment ? <form className={styles.form} onSubmit={(event) => void confirmEnrollment(event)}>
            ...
            <div className={styles.flowActions}>{enrollment.enrollment.state === "PENDING" ? <Button disabled={busy} onClick={() => void cancelEnrollment()} type="button" variant="ghost">{t("cancel")}</Button> : <Button disabled={busy} onClick={restartEnrollment} type="button" variant="ghost">{t("factor.restart")}</Button>}{enrollment.outcome === "APPLIED" ? <Button disabled={busy || totpCode.length !== 6} type="submit">{busy ? t("saving") : t("factor.confirm")}</Button> : null}</div>
          </form> : null}
        </Card.Body>
      </Card>
    </div>
