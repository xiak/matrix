...
export function AccessEnterpriseAccounts({ workspace, onUsers }: { workspace: AccessWorkspace; onUsers(): void }) {
  ...
  return <>
    {selected ? <WorkspaceDetail title={selected.name} onBack={() => setSelectedId(null)} actions={{ primary: { id: "import", label: t("importMembers"), onSelect: () => setImporting(selected) }, secondary: [{ id: "members", label: t("visibleMembers"), onSelect: () => setEditing(selected) }, { id: "disconnect", label: t("disconnectEnterprise"), danger: true, onSelect: () => setDeleting(selected) }] }}>
