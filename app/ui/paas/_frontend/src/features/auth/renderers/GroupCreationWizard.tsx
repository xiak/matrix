...
export function GroupCreationWizard({ workspace, onBack, onDone }: { workspace: AccessWorkspace; onBack(): void; onDone(id: string): void }) {
  ...
  return <div className={styles.root}><ContentPage.Heading title={w("createGroup")} back={{ label: t("back"), parentLabel: w("groups"), disabled: busy, onClick: cancel }} />
    <Wizard label={w("createGroup")} steps={steps.map((key) => ({ id: key, label: t(`steps.${key}`) }))} currentStep={step} onStepChange={(next) => { setError(null); clear(); setStep(next); }} completed={created} busy={busy} formRef={form} onSubmit={submit}
      title={created ? t("created") : t(`steps.${currentStep}`)} description={created ? t("createdHint") : t(`hints.${currentStep}`)} progressLabel={u("stepCount", { current: step + 1, total: 2 })} hint={<><ShieldCheck aria-hidden="true" />{t("mockHint")}</>}
      actions={created ? <Button onClick={() => { const group = workspace.groups.find((entry) => entry.name === name.trim()); if (group) onDone(group.id); else onBack(); }}>{t("viewGroup")}</Button> : <><Button variant="ghost" disabled={busy} onClick={cancel}>{w("cancel")}</Button>{step ? <Button variant="secondary" disabled={busy} onClick={() => setStep(step - 1)}>{a("previousStep")}</Button> : null}<Button type="submit" disabled={busy}>{busy ? t("saving") : step < 1 ? t("next") : w("createGroup")}</Button></>}>
