  ...
  return <div className={styles.root}><ContentPage.Heading title={w("createGroup")} back={{ label: t("back"), parentLabel: w("groups"), disabled: busy, onClick: cancel }} />
    <Wizard label={w("createGroup")} steps={steps.map((key) => ({ id: key, label: t(`steps.${key}`) }))} currentStep={step} onStepChange={(next) => { setError(null); clear(); setStep(next); }} completed={created} busy={busy} formRef={form} onSubmit={submit}
      ...
      {created ? <Alert status="success">{t("createdScope")}</Alert> : <>
        {step === 0 ? <div className={styles.metadata}><FormField id={id + "-name"} label={w("name")} hint={t("nameHint")} error={error ? t(error) : undefined}><Input id={id + "-name"} value={name} maxLength={64} invalid={Boolean(error)} aria-describedby={[id + "-name-hint", error ? id + "-name-error" : ""].filter(Boolean).join(" ")} onChange={(event) => { setName(event.target.value); setError(null); }} /></FormField><FormField id={id + "-description"} label={w("description")}><TextArea id={id + "-description"} rows={3} maxLength={256} value={description} onChange={(event) => setDescription(event.target.value)} /></FormField></div> : null}
