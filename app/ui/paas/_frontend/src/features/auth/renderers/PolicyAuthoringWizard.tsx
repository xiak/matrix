...
export function PolicyAuthoringWizard({ policy, copy = false, method = "visual", workspace, scene, onBack, onDone, doneLabel }: {
  ...
  return <div className={styles.root}>
    <ContentPage.Heading title={editing ? `${w("edit")} · ${policy?.name}` : t(`methods.${method}.title`)} scrollKey={`create-policy:${method}:${policy?.id ?? "new"}`} back={{ label: t("back"), parentLabel: w("policies"), disabled: busy, onClick: cancel }} />
