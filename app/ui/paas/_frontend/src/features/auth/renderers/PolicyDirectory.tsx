      ...
      <Tabs.Content value="policies">
        <Tabs.Root value={view.kind} onValueChange={(kind) => change({ kind, service: "all", category: "all" })}>
          ...
          <Tabs.Content className={styles.directoryContent} value={view.kind}>
            ...
            <Table aria-label={w("policies")} aria-busy={deferredQuery !== view.query} className={styles.policyTable} data-custom-only={customOnly || undefined} mobileLayout="stack">
              ...
              <tbody>{visible.map(({ policy, description, services }) => <tr key={policy.id} data-selected={selection.includes(policy.id) || undefined}>
                ...
                <td data-label={t("policyName")}><div className={styles.policyIdentity}><button className={styles.nameLink} title={policy.name} onClick={() => onOpen(policy.id)}>{policy.name}</button>{view.kind === "all" && policy.kind === "custom" ? <Badge>{w("custom")}</Badge> : null}</div></td>
                {!customOnly ? <><td data-label={t("product")} data-mobile-empty={policy.systemCategory !== "product" || undefined}>{policy.systemCategory === "product" ? <span title={services.map((service) => r(`services.${service}`)).join(" · ")}>{services.map((service) => r(`services.${service}`)).join(" · ")}</span> : "—"}</td><td data-label={t("permissionCategory")} data-mobile-empty={!policy.systemCategory || undefined}>{policy.systemCategory ? t(`categories.${policy.systemCategory}`) : "—"}</td></> : null}
                <td data-label={w("description")}><span className={styles.description} title={description}>{description || "—"}</span></td>
                <td data-label={t("updatedAt")}><WorkspaceTime value={policy.updatedAt} /></td>
              </tr>)}</tbody>
            </Table>
