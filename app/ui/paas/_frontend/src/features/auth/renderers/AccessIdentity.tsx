...
function FederationEditor({ federation, workspace, onClose }: { federation?: FederatedAccount; workspace: AccessWorkspace; onClose(): void }) {
  ...
  return <WorkspaceInlineForm title={federation ? t("edit") + " · " + federation.name : t("createFederation")} backLabel={t(federation ? "backToFederationDetails" : "backToFederationDirectory")} onClose={onClose} onSubmit={async () => Boolean(await access.executeWorkspace({ kind: "save-federation", id: federation?.id, name, subject, providerId, roleId, enabled }))}>
    <FormField id={id + "-name"} label={t("name")}><Input id={id + "-name"} required maxLength={64} value={name} onChange={(event) => setName(event.target.value)} /></FormField>
...
export function AccessFederations({ workspace }: { workspace: AccessWorkspace }) {
  ...
  const createTrigger = useRef<{ focus(): void }>(null);
  const editTrigger = useRef<{ focus(): void }>(null);
  const previousEditing = useRef<FederatedAccount | "new" | null>(null);
