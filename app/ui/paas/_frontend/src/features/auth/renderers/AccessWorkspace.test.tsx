...
describe("CAM-style access workspace", () => {
  ...
  it("checks a synthetic workload before creating a role session and revokes it without changing the current identity", async () => {
    ...
    expect(screen.queryByRole("dialog")).toBeNull();
    let workflow = screen.getByRole("group", { name: "创建体验会话" }), panel = within(workflow);
    await select(user, "调用身份", "devops.matrix.internal");
    await user.click(panel.getByRole("button", { name: "检查信任与权限" }));
    expect((await extension.read("preview-only")).roleSessions).toHaveLength(0);
    vi.mocked(repository.workspace!.execute).mockRejectedValueOnce(new Error("offline"));
    await user.click(panel.getByRole("button", { name: "创建体验会话" }));
    await expectRetainedFailure(workflow);
    expect((await extension.read("preview-only")).roleSessions).toHaveLength(0);
    expect(panel.getByRole("combobox", { name: "调用身份" }).textContent).toBe("devops.matrix.internal");
    await user.click(panel.getByRole("button", { name: "创建体验会话" }));
    await waitFor(() => expect(screen.queryByRole("group", { name: "创建体验会话" })).toBeNull());
    expect(document.activeElement).toBe(screen.getByRole("button", { name: "创建体验会话" }));
    ...
    await user.click(screen.getByRole("button", { name: /会话 .* 的操作/ }));
    await user.click(screen.getByRole("menuitem", { name: "模拟访问" }));
    ...
    expect(screen.queryByRole("dialog")).toBeNull();
    workflow = screen.getByRole("group", { name: "撤销会话" }); panel = within(workflow);
    vi.mocked(repository.workspace!.execute).mockRejectedValueOnce(new Error("offline"));
