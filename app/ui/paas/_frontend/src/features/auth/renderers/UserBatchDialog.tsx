...
export function UserBatchDialog({ action, users, onClose, onCompleted, fallbackFocusRef }: {
  ...
  async function submit() {
    ...
    const targets = users.map((user) => ({ id: user.id, resourceVersion: user.resourceVersion }));
  ...
  return <WorkspaceDialog title={t("title", { action: t(`actions.${action}`), count: users.length })} onClose={onClose} onSubmit={submit}
    ...
    <Table aria-label={t("targets")}><thead><tr><th>{a("user")}</th><th>{a("userType")}</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td>{user.loginName}<small>{user.name}</small></td><td>{a("child")}</td></tr>)}</tbody></Table>
