...
export function AccessRoles({ workspace, scene, entityId, onCreate, onOpen }: { workspace: AccessWorkspace; scene: AccountAccessScene; entityId?: string; onCreate(): void; onOpen(view: AccountAccessView, id?: string): void }) {
  ...
  return <>
    {selected ? <WorkspaceDetail title={selected.name} onBack={() => onOpen("roles")} actions={{ primary: { id: "edit", label: r("editMetadata"), variant: "secondary", onSelect: () => setDialog("metadata") }, secondary: [{ id: "delete", label: t("delete"), danger: true, onSelect: () => setDeleting(selected) }] }}>
