...
import { Alert, Badge, Button, EmptyState, FormField, Input, Select, Table, TablePagination } from "@ui/xiak";
...
export function RoleSessions({ role, workspace, scene, onOpen }: { role: AccessRole; workspace: AccessWorkspace; scene: AccountAccessScene; onOpen(view: AccountAccessView, id?: string): void }) {
  ...
  const [creating, setCreating] = useState(false), [revoking, setRevoking] = useState<AccessRoleSession | null>(null), [page, setPage] = useState(1), [pageSize, setPageSize] = useState(10), [now, setNow] = useState(() => new Date().toISOString());
