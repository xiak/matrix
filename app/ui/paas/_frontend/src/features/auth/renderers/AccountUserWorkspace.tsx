...
export function AccountUserAccessMethods({ user, workspace }: { user: AccountUserScene; workspace: AccessWorkspace | null }) {
  ...
  return <ul className={styles.accessMethods}>
    <li>{t("consoleAccess")} · {profile ? t(profile.consoleAccess ? "enabled" : "disabled") : workspace ? a("accessUnknown") : t("enabled")}</li>
    <li>{t("programmaticAccess")} · {profile ? t(profile.programmaticAccess ? "enabled" : "disabled") : workspace ? a("accessUnknown") : a("accessUnsupported")}</li>
  </ul>;
}
