...
export function AccessCredentials({ workspace, scene, embedded = false }: { workspace: AccessWorkspace; scene: AccountAccessScene; embedded?: boolean }) {
  ...
  return <>
    {selected ? <WorkspaceDetail embedded={embedded} title={selected.id} onBack={() => setSelectedId(null)} actions={{ primary: { id: "status", label: t(selected.enabled ? "disable" : "enable"), variant: "secondary", onSelect: () => setChanging(selected) }, secondary: [{ id: "delete", label: t("delete"), danger: true, disabledReason: selected.enabled ? t("errors.disableFirst") : undefined, onSelect: () => setDeleting(selected) }] }}>
      ...
      <h3 className={styles.stepTitle}>{t("keyLogs")}</h3>{selected.lastUsedAt ? <Table aria-label={t("keyLogs")} mobileLayout="stack"><thead><tr><th scope="col">{t("time")}</th><th scope="col">{t("event")}</th><th scope="col">{t("state")}</th></tr></thead><tbody><tr><td data-label={t("time")}><WorkspaceTime value={selected.lastUsedAt} /></td><td data-label={t("event")}>MOCK · ListResources</td><td data-label={t("state")}><Badge status="success">200 · MOCK</Badge></td></tr></tbody></Table> : <p className={styles.note}>{t("noKeyLogs")}</p>}
    </WorkspaceDetail> : <WorkspaceCollection embedded={embedded} title={t("keys")} description={t("keyHint")} items={workspace.keys.map((key) => ({ ...key, name: key.id }))} keywords={(key) => owner(key) + " " + key.description} filter={{ label: t("state"), options: [{ value: "enabled", label: t("enabled") }, { value: "disabled", label: t("disabled") }], matches: (key, value) => key.enabled === (value === "enabled") }} create={{ label: t("createKey"), onClick: () => setCreating(true) }} columns={[t("keyId"), t("owner"), t("state"), t("lastUsed")]} row={(key) => <><td><button className={styles.userLink} onClick={() => setSelectedId(key.id)}>{key.id}</button><small>{key.description}</small></td><td>{owner(key)}</td><td><Badge status={key.enabled ? "success" : "neutral"}>{t(key.enabled ? "enabled" : "disabled")}</Badge></td><td><WorkspaceTime value={key.lastUsedAt} /></td></>} />}
