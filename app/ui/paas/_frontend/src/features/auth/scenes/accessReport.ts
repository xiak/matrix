...
export function buildAccessReport(kind: "credentials" | "security", workspace: AccessWorkspace, scene: AccountAccessScene, generatedAt: string) {
  return {
    ...
    users: scene.users.map((user) => ({ loginName: user.loginName, status: user.state, roles: user.bindings.map((binding) => binding.role), keyCount: workspace.keys.filter((key) => key.ownerId === user.id).length })),
    ...(kind === "security" ? {
