...
func verifySecurePermissions(path string, directory bool) error {
	...
	if pathComponentIsLink(path, info) || (directory && !info.IsDir()) || (!directory && !info.Mode().IsRegular()) {
	...
	if err != nil || control&windows.SE_DACL_PROTECTED == 0 {
		return errors.New("managed DACL is not protected")
	}
	dacl, _, err := descriptor.DACL()
	if err != nil || dacl == nil || dacl.AceCount == 0 {
		return errors.New("managed DACL grants unexpected principals")
	}
	user, err := windows.GetCurrentProcessToken().GetTokenUser()
	if err != nil {
		return err
	...
	for index := uint32(0); index < uint32(dacl.AceCount); index++ {
		...
		if ace.Header.AceType != windows.ACCESS_ALLOWED_ACE_TYPE {
			return errors.New("managed DACL contains an unexpected access rule")
		}
		aceSID := (*windows.SID)(unsafe.Pointer(&ace.SidStart))
		if !user.User.Sid.Equals(aceSID) {
			return errors.New("managed DACL grants another principal")
		}
	}
	return nil
}
