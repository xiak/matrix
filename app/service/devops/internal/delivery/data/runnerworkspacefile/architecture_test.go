...
func TestRunnerWorkspaceKeepsTransportAndExecutionAuthorityOut(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, imported := range parsed.Imports {
			...
			if forbiddenStandard[importPath] {
				t.Errorf("runner workspace imports forbidden authority %q at %s", importPath, files.Position(imported.Pos()))
				continue
			}
			if strings.HasPrefix(importPath, "github.com/xiak/matrix/") &&
				!allowedMatrix[importPath] {
				t.Errorf("runner workspace crosses Matrix boundary through %q at %s", importPath, files.Position(imported.Pos()))
				continue
			}
			if strings.Contains(importPath, ".") && importPath != "golang.org/x/sys/windows" &&
				!strings.HasPrefix(importPath, "github.com/xiak/matrix/") {
				t.Errorf("runner workspace imports unapproved external package %q at %s", importPath, files.Position(imported.Pos()))
			}
		}
	}
}
