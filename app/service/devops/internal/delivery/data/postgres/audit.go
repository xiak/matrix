...
func (repository *AuditOutboxRepository) Claim(
	...
	if err := devopsv1.ValidateID("audit.workerId", workerID); err != nil {
	...
	err := repository.pool.QueryRow(ctx,
		`SELECT tenant_id, event_id, attempts, fencing_token, lease_expires_at, document
		   FROM delivery.claim_audit_event($1, $2)`,
	...
	decodeErr := decodeDocument("AuditEvent", document, &event)
	if decodeErr == nil {
		decodeErr = auditv1.ValidateEventForSource(auditv1.SourceDevOps, event)
	}
	if decodeErr != nil {
...
func (repository *AuditOutboxRepository) Complete(
	...
	_, err := repository.pool.Exec(ctx,
		`SELECT delivery.complete_audit_event($1, $2, $3, $4, $5, $6, $7)`,
		completion.TenantID, completion.EventID, completion.WorkerID, completion.FencingToken,
		completion.Outcome, retryAt, errorCode,
	)
	if err != nil {
...
func (repository *AuditOutboxRepository) Snapshot(ctx context.Context) (auditdispatch.Snapshot, error) {
	if repository == nil || repository.pool == nil {
	...
	if err := repository.pool.QueryRow(ctx,
		`SELECT pending_count, leased_count, retry_count, delivered_count,
		   ...
		   FROM delivery.audit_outbox_snapshot()`,
	).Scan(
		&value.Pending, &value.Leased, &value.Retry, &value.Delivered,
		&value.DeadLetter, &value.ExpiredLease,
	); err != nil {
		return auditdispatch.Snapshot{}, fmt.Errorf("read Audit outbox snapshot: %w", err)
	}
	if value.Pending < 0 || value.Leased < 0 || value.Retry < 0 || value.Delivered < 0 ||
		value.DeadLetter < 0 || value.ExpiredLease < 0 {
		return auditdispatch.Snapshot{}, errors.New("Audit outbox snapshot contains a negative count")
	}
	return value, nil
}

func (repository *AuditOutboxRepository) Readiness(ctx context.Context) (devopsv1.Readiness, error) {
	if repository == nil || repository.pool == nil || ctx == nil {
		return devopsv1.Readiness{}, errors.New("DevOps Audit worker readiness repository is unavailable")
	}
	return readReadiness(ctx, repository.pool, "SELECT * FROM delivery.worker_readiness()")
}

func validateAuditCompletion(value auditdispatch.Completion) error {
	var problems []error
	problems = append(problems,
		devopsv1.ValidateID("audit.tenantId", string(value.TenantID)),
		devopsv1.ValidateID("audit.eventId", string(value.EventID)),
		devopsv1.ValidateID("audit.workerId", value.WorkerID),
	)
	if value.FencingToken < 1 || value.FencingToken > int64(devopsv1.MaximumContractInteger) {
	...
	case auditdispatch.OutcomeRetry:
		if value.RetryAt.IsZero() || value.RetryAt.Location() != time.UTC ||
			value.RetryAt != value.RetryAt.Round(0) || value.RetryAt.Nanosecond()%1_000 != 0 || value.ErrorCode != "" {
