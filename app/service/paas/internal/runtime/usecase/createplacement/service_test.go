...
func TestCreatePlacementCommitsDecisionAndPendingReservation(t *testing.T) {
	...
	if output.Decision.Outcome != paasv1.PlacementScheduled ||
		output.Decision.RuntimeTargetID != "target-a" ||
		output.Decision.RuntimeTargetResourceVersion != 7 {
	...
	if reservation.TenantID != command.TenantID ||
		...
		reservation.WorkloadReleaseID != command.WorkloadReleaseID ||
		reservation.RuntimeTargetID != output.Decision.RuntimeTargetID ||
...
func TestCreatePlacementRejectsSemanticIdempotencyConflicts(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "release",
			mutate: func(stored *StoredDecision) {
				stored.Decision.WorkloadReleaseID = "release-other"
			},
		},
		{
			name: "policy",
			mutate: func(stored *StoredDecision) {
				stored.Decision.PlacementPolicyID = "policy-other"
			},
		},
	}
...
func TestUnschedulableDecisionHasNoReservation(t *testing.T) {
	snapshot := createPlacementSnapshot()
	snapshot.Policy.Spec.RequiredIsolationClass = paasv1.IsolationPhysicalHost
	snapshot.Pools[0].Spec.AllowedIsolationClasses = []paasv1.IsolationClass{
		paasv1.IsolationPhysicalHost,
	}
	snapshot.Targets[0].Status.SupportedIsolationClasses = []paasv1.IsolationClass{
		paasv1.IsolationPhysicalHost,
	}
