...
func TestRequirementAndCapacityClaimArithmeticRejectsOverflow(t *testing.T) {
	t.Run("component multiplication", func(t *testing.T) {
		...
		input.Snapshot.Release.Spec.Components[0].Replicas = 2
	...
	t.Run("component addition", func(t *testing.T) {
		input := singleTargetInput()
		component := input.Snapshot.Release.Spec.Components[0]
		component.Replicas = 1
		component.Resources.CPUMillis = math.MaxInt64
		second := component
		second.Name = "overflow-two"
		input.Snapshot.Release.Spec.Components = []paasv1.WorkloadComponent{component, second}
		if _, err := mustPlanner(t).Plan(input); err == nil ||
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(input *Input) {
				input.Snapshot.Release.Metadata.Scope.TenantID = "tenant-b"
			},
			match: "release tenant",
		},
		{
			name: "policy tenant",
			mutate: func(input *Input) {
