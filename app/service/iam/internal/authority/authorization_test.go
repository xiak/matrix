...
func TestAccessKeyContextHasNoLoginSessionAndRejectsInconsistentAuthority(t *testing.T) {
	...
	// Only the exact access-key-enabled business actions may preserve the USER
	// grant. Known credential restrictions remain recorded, sanitized denials.
	for name, change := range map[string]func(*AccessKeyContext){
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, signedAt := range []int64{now.Unix(), now.Unix() - 301, now.Unix() + 31} {
				...
				wantAllowed := signedAt == now.Unix() && (name == "current user" || name == "bounded identity")
				if err != nil || result.Allowed != wantAllowed || result.ID != "decision-key-context" || result.PolicyEvidence == nil ||
					iamv1.CheckAuthorizationDecisionForRequest(result.AuthorizationDecision, request) != nil {
					t.Fatal("access-key decision did not preserve its declared carrier boundary", err)
				}
				if wantAllowed {
					wantSubject := &iamv1.Subject{Type: iamv1.SubjectUser, ID: string(value.Principal.ID), AccessKeyID: value.Key.ID}
					if result.TenantID != value.Organization.ID || result.InstallationID != "" || !reflect.DeepEqual(result.Subject, wantSubject) {
						t.Fatal("allowed access-key decision lost its exact account, user, or key lineage")
					}
				} else if result.Subject != nil || result.TenantID != "" || result.InstallationID != "" {
					t.Fatal("known restricted carrier did not remain a sanitized denial")
				}
			}
		})
	}
...
func TestPolicyEvaluationUsesOnlyDeclaredProgramAccess(t *testing.T) {
	now := authorityTestTime()
	...
	if result, _, err := EvaluateAttachedPolicies(now, user.Organization.ID, user.InstallationID, subject, user.Policies, request); err != nil || !result.Allowed {
		t.Fatal("declared access-key carrier did not preserve the current USER grant", err)
	}
	unsupported := policyEvaluationRequestForTest(t, iamv1.ActionPaaSTerminalSessionCreate,
		iamv1.ResourceReference{Kind: iamv1.ResourceTerminalSession, ID: "collection"})
	policies := authorityPoliciesForAction(t, now, unsupported.Action)
	if result, _, err := EvaluateAttachedPolicies(now, user.Organization.ID, user.InstallationID, subject, policies, unsupported); !errors.Is(err, errUnsupportedPolicySubject) || result.Allowed {
		t.Fatal("LOGIN_SESSION-only action implicitly accepted an AccessKey carrier", err)
	}
	for _, bounded := range []bool{false, true} {
		boundary := user.Boundary
		if bounded {
			boundary = userBoundaryForTest(user, policyVersionForTest(t, "key-boundary", iamv1.PolicyAllow, unsupported.Action, iamv1.PolicyResourceAnyInAuthority, ""))
		}
		result, err := decide(user.Organization.ID, user.InstallationID, subject, false, policies, boundary, iamv1.ServicePaaS, unsupported, "decision-key-unsupported", now)
		...
		for _, types := range [][]iamv1.SubjectType{{iamv1.SubjectRole}, {iamv1.SubjectUser, iamv1.SubjectRole}} {
			...
			for index := range profile.Actions {
				if profile.Actions[index].Action == request.Action {
					...
					if len(types) == 1 {
						profile.Actions[index].UserAuthenticationMethods = nil
					}
				}
			}
