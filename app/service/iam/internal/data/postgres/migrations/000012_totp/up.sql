...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM iam.totp_authenticators f
        ...
        WHERE f.enrollment_session_id IS NULL OR f.installation_id<>registration#>>'{scope,installationId}'
...
BEGIN
    ...
    IF NOT iam.totp_custody_contract_ready() OR NOT iam.totp_backup_custody_contract_ready()
        OR (SELECT schema_version FROM iam.readiness())<>37 THEN
...
CREATE INDEX IF NOT EXISTS totp_unrecognized_lineage ON iam.totp_authenticators(id)
    WHERE enrollment_session_id IS NULL;
DO $lineage$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint WHERE conrelid='iam.totp_authenticators'::regclass AND conname='totp_enrollment_lineage') THEN
        ALTER TABLE iam.totp_authenticators ADD CONSTRAINT totp_enrollment_lineage CHECK(
            (enrollment_session_id IS NULL AND enrollment_request_id IS NULL AND enrollment_digest IS NULL
            ...
            OR (enrollment_session_id IS NOT NULL AND enrollment_request_id IS NOT NULL
                ...
                AND expires_at IS NOT NULL AND expires_at=created_at+interval '5 minutes'
...
DECLARE relation_name text; operation_name text;
BEGIN
    FOREACH relation_name IN ARRAY ARRAY['user_mfa_states','authentication_challenges','totp_attempts','mfa_recovery_batches','mfa_recovery_codes'] LOOP
...
-- This slice issues recovery material but does not yet admit consumption or
-- replacement. Until that purpose-bound transition exists, even a trusted
-- writer cannot rewrite an issued verifier, batch identity or consumed state.
...
BEGIN
    FOREACH relation_name IN ARRAY ARRAY['mfa_recovery_batches','mfa_recovery_codes'] LOOP
        ...
        EXECUTE format('CREATE TRIGGER cannot_update BEFORE UPDATE ON iam.%I FOR EACH ROW EXECUTE FUNCTION iam.reject_policy_history_change()',relation_name);
...
BEGIN
    ...
    IF result.enrollment_state='NEVER_BOUND' AND EXISTS(SELECT 1 FROM iam.totp_authenticators f
        WHERE f.tenant_id=tenant AND f.user_id=subject_id AND (f.state='ACTIVE' OR f.enrollment_outcome='CONFIRMED' OR f.enrollment_session_id IS NULL)) THEN
    ...
    SELECT COALESCE((SELECT
        (m.enrollment_state='NEVER_BOUND' AND m.revision=1
            AND NOT EXISTS(SELECT 1 FROM iam.totp_authenticators f WHERE f.tenant_id=m.tenant_id AND f.user_id=m.user_id
                AND (f.state='ACTIVE' OR f.enrollment_outcome='CONFIRMED' OR f.enrollment_session_id IS NULL))
...
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,pg_temp AS $function$
BEGIN
    IF OLD.enrollment_session_id IS NULL OR OLD.state='REVOKED'
...
DECLARE state iam.user_mfa_states%ROWTYPE; generation bigint; effective_now timestamptz(6);
    account_version bigint; principal_version bigint;
BEGIN
    ...
    END IF;
    state:=iam.lock_mfa_user(tenant,subject_id);
    IF state.enrollment_state<>'BOUND' THEN
    ...
    INSERT INTO iam.authentication_challenges(tenant_id,id,user_id,lookup_digest,verification_digest,credential_generation,
        password_attempt_id,password_attempt_sequence,account_version,principal_version,mfa_revision,factor_id,request_id,request_digest,state,created_at,expires_at)
        VALUES(tenant,challenge_id,subject_id,lookup_digest,verification_digest,generation,attempt,attempt_sequence,account_version,principal_version,
            state.revision,state.factor_id,request_id,request_digest,'PENDING',effective_now,effective_now+interval '5 minutes');
    RETURN jsonb_build_object('apiVersion','iam.matrix.xiak.com/v1','kind','AuthenticationChallenge',
        'id',challenge_id,'purpose','LOGIN','nextStep','TOTP','expiresAt',effective_now+interval '5 minutes');
END $function$;
...
BEGIN
    IF purpose IS NULL OR NOT ((purpose='ENROLLMENT' AND caller_id IS NOT NULL) OR (purpose='LOGIN' AND caller_id IS NULL)) THEN
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $function$
DECLARE factor iam.totp_authenticators%ROWTYPE; attempt iam.totp_attempts%ROWTYPE; effective_now timestamptz(6);
BEGIN
    factor:=iam.lock_totp_verification(tenant,subject_id,caller_id,reference_id,purpose);
    SELECT * INTO attempt FROM iam.totp_attempts a WHERE a.tenant_id=tenant AND a.user_id=subject_id FOR UPDATE;
    effective_now:=clock_timestamp();
    IF NOT FOUND OR attempt.attempt_id IS DISTINCT FROM read_totp_attempt.attempt_id OR attempt.sequence IS DISTINCT FROM attempt_sequence
    ...
    RETURN jsonb_build_object('factorId',factor.id,'installationId',factor.installation_id,'keyId',factor.key_id,'formatVersion',factor.format_version,
        ...
        'databaseTime',effective_now,'factorRevision',attempt.mfa_revision,
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $function$
DECLARE factor iam.totp_authenticators%ROWTYPE; contact record; code jsonb; next_revision bigint;
BEGIN
...
DECLARE tenant text; binding_event_id text; factor iam.totp_authenticators%ROWTYPE; batch iam.mfa_recovery_batches%ROWTYPE;
    notice record; original_contact record; event jsonb;
BEGIN
    ...
    CASE TG_TABLE_NAME
        ...
        WHEN 'audit_outbox' THEN
            IF NEW.event_document->>'action' IS DISTINCT FROM 'iam.authenticator.bound' THEN RETURN NULL; END IF;
            binding_event_id:=NEW.event_id;
        WHEN 'security_notifications' THEN
            IF NEW.kind<>'AUTHENTICATOR_BOUND' THEN RETURN NULL; END IF;
            binding_event_id:=NEW.event_id;
    ...
    IF NOT FOUND OR factor.enrollment_outcome<>'CONFIRMED' OR factor.bound_revision IS NULL
        OR NOT EXISTS(SELECT 1 FROM iam.sessions s WHERE s.tenant_id=tenant AND s.id=factor.enrollment_session_id AND s.principal_id=factor.user_id)
    ...
    IF event IS NULL OR event->>'action' IS DISTINCT FROM 'iam.authenticator.bound'
    ...
    SELECT n.* INTO notice FROM iam.security_notifications n
        WHERE n.tenant_id=tenant AND n.event_id=binding_event_id AND n.kind='AUTHENTICATOR_BOUND';
...
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,pg_temp AS $function$
BEGIN
    IF OLD.state<>'PENDING' OR (to_jsonb(NEW)-ARRAY['state','attempts','completed_at','session_id','issuance_event_id','password_challenge_id','verified_step','password_changed_event_id','password_change_request_id'])
        IS DISTINCT FROM (to_jsonb(OLD)-ARRAY['state','attempts','completed_at','session_id','issuance_event_id','password_challenge_id','verified_step','password_changed_event_id','password_change_request_id'])
        OR (OLD.next_step='PASSWORD_CHANGE' AND NEW.verified_step IS DISTINCT FROM OLD.verified_step) THEN
...
BEGIN
    FOR expected IN SELECT * FROM (VALUES
        ('user_mfa_states','tenant_id,user_id,revision,enrollment_state,factor_id','text,text,bigint,text,text','factor_id','tenant_id,user_id,factor_id',true),
        ('authentication_challenges','tenant_id,id,user_id,lookup_digest,verification_digest,credential_generation,password_attempt_id,password_attempt_sequence,account_version,principal_version,mfa_revision,factor_id,request_id,request_digest,next_step,source_challenge_id,password_challenge_id,verified_step,password_changed_event_id,password_change_request_id,state,created_at,expires_at,attempts,completed_at,session_id,issuance_event_id',
            'text,text,text,text,text,bigint,text,bigint,bigint,bigint,bigint,text,text,text,text,text,text,bigint,text,text,text,timestamptz,timestamptz,integer,timestamptz,text,text',
            'source_challenge_id,password_challenge_id,verified_step,password_changed_event_id,password_change_request_id,completed_at,session_id,issuance_event_id',
            'tenant_id,id,user_id,password_attempt_id,factor_id,request_id,source_challenge_id,password_challenge_id,password_changed_event_id,password_change_request_id,session_id,issuance_event_id',true),
        ...
        ('mfa_recovery_batches','tenant_id,user_id,id,factor_id,mfa_revision,event_id,created_at,revoked_at',
            'text,text,text,text,bigint,text,timestamptz,timestamptz','revoked_at','tenant_id,user_id,id,factor_id,event_id',true),
        ('mfa_recovery_codes','tenant_id,batch_id,id,verification_digest,consumed_at',
            'text,text,text,text,timestamptz','consumed_at','tenant_id,batch_id,id',true),
        ('totp_authenticators','enrollment_session_id,enrollment_request_id,enrollment_digest,enrollment_revision,credential_generation,expires_at,enrollment_outcome,completed_at,bound_revision,bound_event_id,revoked_at',
            'text,text,text,bigint,bigint,timestamptz,text,timestamptz,bigint,text,timestamptz',
            'enrollment_session_id,enrollment_request_id,enrollment_digest,enrollment_revision,credential_generation,expires_at,enrollment_outcome,completed_at,bound_revision,bound_event_id,revoked_at',
            'enrollment_session_id,enrollment_request_id,bound_event_id',false),
    ...
    END LOOP;
    FOR expected IN SELECT * FROM (VALUES
        ('user_mfa_states','user_mfa_states_revision_check,user_mfa_states_enrollment_state_check,user_mfa_states_shape,user_mfa_states_never_bound'),
        ...
        ('mfa_recovery_batches','mfa_recovery_batches_mfa_revision_check'),
        ('mfa_recovery_codes','mfa_recovery_codes_verification_digest_check'),
    ...
    FOR expected IN SELECT * FROM (VALUES
        ...
        ('totp_authenticators','totp_unrecognized_lineage','id',false,'(enrollment_session_id IS NULL)'),
    ...
    FOR expected IN SELECT * FROM (
        SELECT relation_name,'cannot_delete'::text AS name,'iam.reject_policy_history_change()'::text AS signature,11 AS event_type,false AS deferred
            FROM unnest(ARRAY['user_mfa_states','authentication_challenges','totp_attempts','mfa_recovery_batches','mfa_recovery_codes']) r(relation_name)
        ...
        UNION ALL SELECT * FROM (VALUES
            ...
            ('mfa_recovery_batches','cannot_update','iam.reject_policy_history_change()',19,false),
            ('mfa_recovery_codes','cannot_update','iam.reject_policy_history_change()',19,false),
