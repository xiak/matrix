...
func HistoricalAuthorizationProfiles() []AuthorizationProfile {
	return []AuthorizationProfile{
		cloneAuthorizationProfile(iamProfileRevisionOne),
...
func paasAccessKeyProfile() AuthorizationProfile {
	profile := roleBusinessProfile(paasProfileRevisionOne)
	profile.Revision = 3
	profile.Actions = append(profile.Actions,
		declaredProfileAction(ActionPaaSTerminalSessionCreate, ResourceTerminalSession, AuthorityScopeTenant, ResourceTerminalSession, []AuthorizationResourceShape{{Mode: AuthorizationResourceCollection, CollectionUsage: AuthorizationCollectionCreate}}),
		declaredProfileAction(ActionPaaSTerminalSessionClose, ResourceTerminalSession, AuthorityScopeTenant, "", []AuthorizationResourceShape{{Mode: AuthorizationResourceInstance}}),
	)
	for index := range profile.Actions {
		profile.Actions[index].SubjectTypes = []SubjectType{SubjectUser}
		if profile.Actions[index].Scope == AuthorityScopeTenant {
		...
		if profile.Actions[index].Action == ActionPaaSDeploymentRead ||
			profile.Actions[index].Action == ActionPaaSNodeEnrollmentRead {
		...
		switch profile.Actions[index].Action {
		...
		case ActionPaaSApplicationCreate, ActionPaaSApplicationRead,
			ActionPaaSConfigurationCreate, ActionPaaSConfigurationRead,
			ActionPaaSConfigurationRevisionCreate, ActionPaaSConfigurationRevisionRead,
			ActionPaaSApplicationRevisionCreate, ActionPaaSApplicationRevisionRead,
			ActionPaaSDeploymentCreate, ActionPaaSDeploymentUpdate,
			ActionPaaSDeploymentRollback, ActionPaaSDeploymentStop,
			ActionPaaSDeploymentRead, ActionPaaSOperationRead:
			profile.Actions[index].UserAuthenticationMethods = []UserAuthenticationMethod{
				UserAuthenticationLoginSession,
				UserAuthenticationAccessKey,
			}
		}
	}
	return profile
...
func auditAccessKeyProfile() AuthorizationProfile {
	profile := roleBusinessProfile(auditProfileRevisionOne)
	profile.Revision = 3
	for index := range profile.Actions {
		switch profile.Actions[index].Action {
		case ActionAuditRecordRead, ActionAuditIntegrityVerify:
			profile.Actions[index].UserAuthenticationMethods = []UserAuthenticationMethod{
				UserAuthenticationLoginSession,
				UserAuthenticationAccessKey,
			}
