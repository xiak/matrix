...
func TestOpenAPINodeEnrollmentExchangeIsBoundedBootstrapOnly(t *testing.T) {
	...
	for _, name := range []string{
		...
		contract := schemaObject(t, schemas, name)
		if contract["x-matrix-visibility"] != "node-bootstrap" || contract["additionalProperties"] != false {
			t.Errorf("%s is not a closed node-bootstrap contract: %#v", name, contract)
		}
	}
	challenge := schemaObject(t, schemas, "NodeEnrollmentRecoveryChallenge")
	challengeProperties := object(t, challenge["properties"], "NodeEnrollmentRecoveryChallenge.properties")
	for _, name := range []string{"challenge", "authenticator"} {
		property := object(t, challengeProperties[name], "NodeEnrollmentRecoveryChallenge."+name)
		if property["minLength"] != json.Number("43") || property["maxLength"] != json.Number("43") {
			t.Errorf("%s must be fixed-length base64url: %#v", name, property)
		}
	}
	proof := schemaObject(t, schemas, "RecoverNodeEnrollmentExchangeRequest")
	proofProperties := object(t, proof["properties"], "RecoverNodeEnrollmentExchangeRequest.properties")
	for _, name := range []string{"nodeSignature", "collectorSignature"} {
		property := object(t, proofProperties[name], "RecoverNodeEnrollmentExchangeRequest."+name)
		if property["writeOnly"] != true || property["minLength"] != json.Number("86") || property["maxLength"] != json.Number("86") {
			t.Errorf("%s must be a bounded request-only proof: %#v", name, property)
		}
	}
	paths := object(t, document["paths"], "paths")
	for _, path := range []string{
		"/v1/node-enrollments/{nodeEnrollmentId}/recovery-challenge",
		"/v1/node-enrollments/{nodeEnrollmentId}/recover",
	} {
		operation := object(t, object(t, paths[path], path)["post"], path+".post")
		security, ok := operation["security"].([]any)
		if !ok || len(security) != 0 {
			t.Errorf("%s must not accept an IAM session: %#v", path, operation["security"])
		}
	}
}
