...
- 状态：CAT-01–04 首片已验收（固定 `3b11eb9`）；CAT-05 的源码 Profile、不可变注册、当前一致性及请求/决定绑定已有固定验证，编译版本及冻结动作族的运行证据归005。CAT-06 只读编辑目录固定 `40407e27` 的真实门禁及独立CI已通过；CAT-07 主体能力契约/当前评估已有本地整仓与真库/多进程证据，测试隔离修复后的固定`d45402d9`三项独立CI通过。CAT-08 纯载体契约已有本地整仓与模糊测试，尚未接入当前业务声明和真实授权；ROLE业务运行归006，AccessKey归007。最终发布消费者组合仍待验收，本 FEAT 整体未验收。
...
| IAM-CAT-08 | USER 的认证载体由动作显式声明；历史缺字段仅支持登录会话，AccessKey 必须另经当前目录准入、真实凭据与原策略求值，不从 USER 类型推导 |
...
`AuthorizationProfileAction.userAuthenticationMethods` 与 subjectTypes 正交，只描述该动作支持的 USER 凭据类型，不是新主体、权限或会话。集合闭合为 LOGIN_SESSION、ACCESS_KEY；显式集合非空、最多两项且无重复，规范编码排序并纳入原 Profile 摘要。只有动作确实支持 USER 时才可声明；ROLE 和 SERVICE_ACCOUNT 不继承它。未知/null/空/大小写别名拒绝，深拷贝、完整内容比较及源码快速路径必须覆盖该字段。

旧声明缺字段保持原 bytes/digest，只解释为 LOGIN_SESSION；不能回填或因当前目录支持 USER 就启用 AccessKey。纯检查绑定准确 Profile 引用、动作和认证载体，不验证密钥/nonce、不执行 PDP、不产生 permit。当前源码声明保持不变，后续与007实际业务入口一同采用新 revision；只有元数据通过不等于已开放程序访问。

AccessKey 使用旧 USER 策略的最窄兼容规则归007的原子授权片：仍核对真实冻结/当前声明，除载体从缺省/LOGIN_SESSION增加ACCESS_KEY外，请求动作的产品、调用服务、scope、resourceKind、全部目标形状/用途、subjectTypes及原有条件等语义必须相同。不能全局忽略 digest，不能跳过不兼容 Deny；LOGIN_SESSION 的既有解释保持。历史决定保留当时两份声明，不用今天的目录重新授权。

验收沿既有契约/schema及007真实请求 owner：集合规范/克隆隔离、所有历史源码声明 bytes不变、USER与ROLE/SERVICE交叉拒绝、错误引用/未知动作/缺声明拒绝，以及当前载体不支持的合法签名仍同事务记录Deny和消费nonce。前一组纯契约不替代后一组真实授权、持久化与产品PEP验收。

2026-09-17，纯契约干净树`26715a9bd17e6caf2ed372cb20679d1b83c37220`通过全仓race/p2（含架构）、vet、模块验证、生成文件集合/字节稳定和Linux amd64构建。原Profile模糊门禁增加显式载体种子及往返后的准入等值断言，2-worker/20秒预算实际22.134秒、181913次执行通过；原API/schema owner覆盖null、空/重复/未知集合、非USER主体、legacy probe、借用源码摘要和克隆变更。字段只纳入声明契约，未推进当前Profile/revision、数据库或服务准入；007的原策略编译兼容证据不替代真实请求/nonce验收。
...
固定`7b597101c55cbd782e611f8da794fc84da138ebb`的[Verification35067085558](https://github.com/xiak/matrix/actions/runs/35067085558)最终为failure：Go/节点成功，authority-process的IAM附件会话和HTTP聚合测试分别耗尽120秒/180秒上下文；同一作业内Audit、独立进程87.063秒及PaaS数据库均通过。测试隔离修复后的固定`d45402d91c89a5bb23f52fcfde65491435cd0f55`已推送，GitHub API核实[Verification35069879250](https://github.com/xiak/matrix/actions/runs/35069879250)精确SHA的三项全部success；生产主体能力代码未因测试拆分改变，具体测试范围归006/011。不能将旧失败改写为成功，也不能将该候选当作ROLE运行身份验收。本地只清理先前客户端已归零的自有PG容器和已空专属网络，合成数据卷保留；未操作其他任务或远端环境。
