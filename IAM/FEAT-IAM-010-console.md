...
2026-09-20，前端实现固定在已推送的
[`6d8825a3`](https://github.com/xiak/matrix/commit/6d8825a3daa6379bdc07621a4e0b2fcc32b21288)，
...
- 提交后响应丢失会保存原始意图并进入跨路由保留的 `UNKNOWN` 锁定态；预览仓库真正 reject/超时时也会先把同一非秘密意图记入隔离恢复日志，再向页面暴露 UNKNOWN。页面不播报成功、不允许第二次保存，也不复用密码、TOTP 或操作证明。按同一原始意图查询时，`NOT_FOUND` 与查询暂不可用都继续保持未知；只有权威的已应用或已拒绝结果才解除锁定。已拒绝结果保持原规则，下一次修改必须建立新的审阅与证明。
...
- 账户工作区与预览仓库 191 条定向行为用例、完整前端 40 个文件/608 条用例、三条静态归一化、228 组主题对比、40 页生产导出、222 个嵌入文件及 `go test ./...`、`go vet ./...` 均通过。新增用例从仓库真实 reject 进入 UNKNOWN，切页并重新读取后仍保持原意图锁定。完整共享门禁由 [FEAT-007 current development evidence](../docs/features/FEAT-007-control-plane-console.md#current-shared-navigation-development-evidence) 唯一拥有；本证据不宣称 IAM-009 已有公开 HTTP、真实持久化、会话失效、权威未知结果查询或后端执行验收。
