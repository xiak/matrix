...
登录页 LIVE 客户端当前只消费固定的 `CHALLENGE_REQUIRED/TOTP` 与验证后的 `PASSWORD_CHANGE`，不提供跳过前置条件的首次绑定捷径。恢复材料按规范材料逐字、区分大小写匹配，客户端不能 trim、改写大小写或暴露精确失败原因；恢复一旦开始，取消只能返回普通登录/重新认证，不能回到旧挑战继续。恢复材料只能进入受限重绑，不能直接签发普通登录会话；管理员不能代替本人重置因子。自助恢复的新后端增量尚未形成 UI 可消费的固定公开契约，因此恢复仍只保留明确标识的隔离 MOCK，不进入 LIVE 登录适配。
...
[`06e5b8c7576a77ed4ba7009fc1f46f294570a19b`](https://github.com/xiak/matrix/commit/06e5b8c7576a77ed4ba7009fc1f46f294570a19b)，
...
- Provider 行为证明挑战开始时清空 credential/current session，挑战材料只保存在内存；验证失败、到期与取消都不能创建会话。验证成功才建立 session；验证后改密成功会清空状态并要求使用新密码重新登录，不调用会话内 `/auth/password`。
...
- 完整前端 40 个测试文件、617 条用例及三条静态归一化用例通过；类型、lint、架构、228 组主题对比、40 页生产导出与 223 个嵌入文件等价通过。真实 DEV 在桌面与 `390 × 844`、混色与深色下验证登录挑战和验证后改密页面，无横向溢出或新增 warning/error。
