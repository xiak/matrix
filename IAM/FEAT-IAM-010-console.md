...
所有。真实 PolicyVersion/授权目录作者流程、Role/trust/STS、SSO 与个人 MFA 的 LIVE 适配，访问密钥的真实后端浏览器联调，以及本边界片的浅色/混色浏览器与键盘专项尚未完成。仍需与固定后端原子整合并执行相应发布门禁；本 UX 分支独立运行不等于完整 IAM 安装候选已验收。
