...
- 绑定、替换与合法移除写入共享 MOCK workspace 的重新认证状态。设置页卸载、切换左侧菜单、普通 logout/login 都不清除已提交因子或恢复历史；只有显式“重置体验数据”边界重建初始 MOCK。重新认证期间账号、Workspace、批量用户、用户组和权限边界的管理写入口共同拒绝旧会话；一次性材料页结束后旧会话失效。
- 登录恢复输入保持原始大小写，不 trim 或自动大写；错误只使用通用提示。恢复材料验证成功便把共享状态写为 `RECOVERY_REQUIRED/rebind-required` 并使旧因子不可再用；取消返回普通登录，重新打开仍直接进入重绑，不能复活旧 MFA 挑战。新 TOTP 确认即提交替代因子，后续恢复材料确认不签发会话；回到普通登录后仍须密码和新 TOTP 才能创建新会话。
- 组件和仓库行为用例覆盖提交点、一次性材料离开边界、普通退出保留、跨重开恢复状态、精确材料匹配、不可覆盖重放与所有管理写入口的重新认证闸门。完整前端 40 个测试文件、600 条用例、生产导出、222 个嵌入文件、`go test ./...` 和 `go vet ./...` 均通过。真实 DEV 完成普通退出后 MFA、恢复取消再打开、重绑后再次登录；干净重载后新增 warning/error 为空。
