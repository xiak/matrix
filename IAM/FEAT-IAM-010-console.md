...
设置页固定标题、边界说明和卡片外壳同步渲染，只有联系地址和认证器数据区使用局部加载反馈；确认或刷新数据时不卸载整个设置页。DEV 一键体验继续使用原独立、明确标记的页内 MOCK，不调用 IAM、不写浏览器存储，也不发送真实邮件。
...
账号级 MFA 要求遵循 IAM-009 S2/S3 的固定设计来源 `d570673ba87c113f7474fdab79b5e22a83c2b323`。固定运行时来源 `018fbd7505ae16d59dcfe857df7fbbe870bf1a13` 已提供 `GET /v1/account/security-settings` 和精确当前 Account 实例的 `iam.security-settings.read`；写入、设置专用 step-up 与历史读取尚未实现。非体验设置页只读取当前账号规则，严格核对版本、归属、显式布尔值和更新时间，不带 Account selector，不由旧系统策略或 Root 名称猜测读取权限。401 仅过期发起请求的会话；403 是局部无权，不清除登录态；404 和其他失败不回退 MOCK。固定标题与卡片保持，只有规则数据区加载或报错；`requiredForUsers=false` 不代表个人未绑定因子或当前会话未完成 MFA。

独立 DEV 体验把规则作为不同于本人认证器、恢复代码与安全通知地址的 Account 设置呈现：模拟编辑后先审阅准确的 Account 目标、适用身份和会话影响，再完成仅绑定本次规则变更的密码加 TOTP step-up，最后只写入隔离仓库。step-up 只证明认证强度，不授予 IAM 权限；真实保存仍须由后端重新鉴权，任何真实读取或写入失败都不得回退 MOCK。
...
创建页与版本作者复用同一 JSON/可视化编辑边界。JSON 允许完整声明，目录不可用时仍可用；可视化只在主动选择后读取当前权限能力目录，用户须明确选取精确 TENANT Action。新实例声明默认要求填写 `EXACT` 资源 ID，集合型 Action 的精确目标才默认字面量 `collection`；扩大到 `ANY_IN_AUTHORITY` 必须由用户主动选择。切换编辑模式不得丢弃无法无损呈现的 JSON。审阅显示名称、完整声明与“创建不等于授权”，成功显示服务端稳定 ID，再由用户进入详情；显式 MOCK 四方法仍独立保留。
