...
- User 详情的所属用户组与直接策略关系在 `0ea49e06fbfcd9e0cf5e676207a1d2fe0ad8244b` 统一为内容区“选择 → 审阅 → 完成”工作流，不再用大型 Dialog。审阅分别列出新增、移除和保持关系，并明确直接策略、组继承及权限边界不会互相冒充；提交失败保留原选择和审阅，重试调用同一现有 MOCK 命令。`539px` DEV 审阅页无 Dialog 或页面横向溢出，焦点进入审阅标题。该片不增加 LIVE API 或授权推断。
