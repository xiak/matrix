...
真实策略版本读取、切默认与退休已有前端客户端，但仍需与固定 IAM 进程完成浏览器联调；当前目录元数据和边界引用不能替代这些权威结果。真实版本发布与策略作者表单仍待独立接入和验证，不能由切默认路由推导出发布能力。只读权限能力目录客户端已经接入固定契约，但仍需真实进程浏览器验收；产品声明管理不是租户策略功能。
