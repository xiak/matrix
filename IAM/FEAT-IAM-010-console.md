...
### 首次登录强制改密 MOCK 的开发验收证据

2026-09-20，前端实现固定在已推送的
[`60b1bca5`](https://github.com/xiak/matrix/commit/60b1bca5)。

- 初始密码只能建立强制改密受限会话；改密成功后该 credential 立即失效，前端清空身份并回到普通登录页，明确告知使用新密码重新登录。受限 credential 不会原地升级为正常控制台会话，也不会自动返回原业务页面。
- 改密失败仍停留在受限场景；成功提示只描述已完成的密码更新和会话结束，不声称后端已实现会话族撤销、通知投递或其他尚未固定的安全能力。预览仓库为下一次登录签发新的 opaque credential，旧 credential 后续读取返回 401。
- Provider、登录渲染器和预览仓库行为用例共同覆盖成功、失败、原目标页返回和旧 credential 失效。完整前端 40 个测试文件、603 条用例、三条静态归一化用例、228 组主题对比、40 页生产导出、222 个嵌入文件、`go test ./...` 与 `go vet ./...` 均通过；真实 DEV 一键 MOCK 登录、控制台跳转与干净重载通过，新增 warning/error 为空。
- 本证据只接受隔离 MOCK 的客户端状态机。真实密码写入、初始密码生命周期、失败计数、并发冲突、凭证撤销和重新登录后的权威身份读取仍由 IAM 固定公开契约与其真实运行验收负责。
