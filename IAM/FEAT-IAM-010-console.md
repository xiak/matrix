...
- 账户工作区与预览仓库定向行为用例、完整前端 43 个文件/693 条用例、三条静态归一化、228 组主题对比、40 页生产导出、223 个嵌入文件及 `go test -p 2 ./...`、`go vet -p 2 ./...` 均通过。用例覆盖无权读取、跨 Account/缺字段响应、局部重试、迟到旧 Session 响应与同 bearer 迟到 401，以及 MOCK 的未绑定拒绝、UNKNOWN 和不可恢复写锁。完整共享门禁由 [FEAT-007 current development evidence](../docs/features/FEAT-007-control-plane-console.md#current-shared-navigation-development-evidence) 唯一拥有；本证据不宣称 LIVE 写入、设置专用 step-up、历史/未知结果查询或真实 IAM 浏览器与发布验收已完成。
