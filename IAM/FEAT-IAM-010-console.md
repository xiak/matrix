...
[`67d22770a98d69d08892e920b756bae6dc649d88`](https://github.com/xiak/matrix/commit/67d22770a98d69d08892e920b756bae6dc649d88)。
...
- 原型只引用已有 `MatrixDeliveryAccess` 的精确默认版本和 Action 模式，明确展示跨 PaaS/DevOps 代操作。产品接入、callingService、服务身份认证与客户同意保持独立；目录可见性和产品安装不授权。模板升版不得自动扩权，未来只应使用专用角色的短期会话，撤销前必须检查关联资源。
...
- 行为用例证明全程无 Dialog、无 repository/workspace 写调用、原 MOCK 状态逐字段不变，并覆盖进入、阶段切换、返回焦点与准确策略快照。完整前端 42 个测试文件、659 条用例及三条静态归一化用例通过；类型、lint、架构、228 组主题对比、40 页生产导出、223 个嵌入文件等价、`go test -p 2 ./...` 与 `go vet -p 2 ./...` 通过。真实 DEV 在默认紧凑视口和 `360 × 800` 完成目录、详情与三步审阅；小屏 document/body 均为 `clientWidth === scrollWidth === 360`，Dialog 为零，阶段标题滚回可见且日志无 warning/error。
