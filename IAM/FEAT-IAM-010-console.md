...
Secret 只在创建结果明确为 `APPLIED` 时展示一次，并在确认离开后从 DOM 与页面状态移除。网络或 5xx 导致结果未知时必须锁定原 requestId 并查询原请求；若已创建但 Secret 已丢失，先停用并删除该密钥，之后才允许以新 requestId 创建新意图。当前固定契约没有可靠的最近使用证据，因此页面不能制造 last-used 时间、活跃工作负载或“可安全删除”结论。
...
2026-09-20，前端实现固定在已推送的
[`4901f428`](https://github.com/xiak/matrix/commit/4901f428)，
...
- 可执行的“响应丢失”路径锁定原 requestId，原请求查询只返回非秘密密钥事实；若密钥已经存在但 Secret 不可恢复，页面要求先停用和删除该密钥，之后才开放新 requestId。行为用例同时覆盖创建失败重试、状态转换、删除前置条件和本地存储不含 Secret。
- 轮换指导固定为“创建第二把 → 外部验证 → 停用旧密钥 → 删除旧密钥”，并明确密码修改、退出登录、会话撤销与访问密钥生命周期相互独立，产品 PEP 尚未接入。
- 完整前端 40 个测试文件、591 条用例与三条静态归一化用例通过；类型、lint、架构、228 组主题对比、40 页生产导出、223 个嵌入文件等价及 Go UI 宿主门禁通过。真实 DEV 在 `360 × 800` 验证内容区审阅，document `clientWidth == scrollWidth == 360`、Dialog 数为零。浏览器日志仅保留创建文件时 19:42 的旧 HMR 模块缺失记录；19:49 完整生产构建成功且没有新运行时告警。
...
所有。真实 PolicyVersion/授权目录作者流程、Role/trust/STS、SSO、访问密钥 LIVE 适配、个人 MFA，以及本边界片的浅色/混色浏览器与键盘专项尚未完成。仍需与固定后端原子整合并执行相应发布门禁；本 UX 分支独立运行不等于完整 IAM 安装候选已验收。
