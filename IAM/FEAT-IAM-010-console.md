...
策略目录保留桌面列、搜索、筛选、分页和至多 30 项的当前结果选择。窄容器下以同一原生表格的带字段标签纵向行呈现策略元数据；没有产品/级别值的 MOCK 自定义策略只在小屏省去空字段，不把“—”误当权限分类。LIVE 元数据目录沿用相同响应式契约，数据归属、授权与正文懒读取规则不因布局变化。

用户目录在桌面保留身份、类型、访问方式、策略来源和状态列；小于或等于 `560px` 的表格容器把同一原生行堆叠为可读的纵向信息，不要求横向拖动。选择框、用户身份与状态位于行首，访问方式和策略关联带字段标签；主身份继续独立于普通 User 表格展示。响应式排版不得改变本页选择范围、能力门控或分页语义。
