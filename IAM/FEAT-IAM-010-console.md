...
真实组详情的成员关系是独立读取区：首次读取才使用局部骨架；组变更后的重新读取保留已验证的旧行并标明更新中，期间关闭关系操作，避免整表先清空再闪回。若权威组详情读取失败则关闭旧详情和操作；若仅成员重读失败则隐藏旧行并提供局部重试，不能把过期关系当作当前成功结果。
...
- 组详情的首次成员读取仍只在成员区显示局部骨架；权威组详情重读期间保留已验证的详情与成员行，显示更新提示并关闭编辑、删除及关系命令。成员重读成功后原位替换；失败时隐藏旧行并允许局部重试；组详情重读失败时整个旧详情不再可操作。切换准确组 ID 会重新建立详情状态，避免跨对象复用旧成员。
...
- 2026-09-25 再以本 UX 分支当前完整静态 UI 二进制，配对 IAM 工程师提供的固定后端 `29668fa330b2233b43ebd48ed37738623377f9de`，在独立 PostgreSQL 18.6 合成 fixture 完成真实浏览器联调。先实看策略元数据目录和 `Browser boundary A` 的默认 v2 冻结 Action 声明，再从 User 目录进入 `browser.member`，在页面中审阅并执行边界 A → B → 无边界；每次都核对服务端重读的 Policy 引用、默认版本和 User revision 2 → 3 → 4 → 5，原 `PaaSDeveloper` 直接附件不变。页面观察后才写 fixture completion marker；`go test -race -p 1 ./test/authorityprocess -run '^TestIAMConsoleBrowser$' -count=1 -timeout 35m -v` 通过，后端最终核对两个设置事实、一个移除事实及 Audit 链。该轮只验证这些桌面真实路径；策略发布/关联、组写入、角色、SSO、窄屏和发布安装仍不能从此结果推断通过。临时数据库及合成数据已删除，DEV MOCK 4317 仍独立可检查。
