...
- 状态：S1后端固定`3080922f6ae1871f1c351d5ee30f03551fc3c605`已通过本地累计真库、保留数据、独立进程、全仓检查及五项独立CI，本人会话目录/逐个撤销/精确重放已验收。S1b一键结束其他会话的后端实现和本地累计门禁已完成，累计独立CI尚未通过、未验收；一般事务分组及失败证据归011，既有Role管理门禁的准备复用归006。当前候选源码IAM32/Audit19，未分配发布revision。UI与完整009尚未验收。S2本地MFA、S3安全规则/认证预算、S4安全报告/闲置治理仍仅设计，尚未冻结全部材料、权限和公共契约，未实施/未验收。
- 依赖：003、005、007。
...
本片后端固定`159bb302fed89161c4b60afeb4a6f41f9bd99820`实现及本地门禁已完成，累计独立CI尚未通过、未宣告验收，补齐IAM-SEC-02的批量自助减权；不代替S2–S4、UI或完整009。已验证并推送的`3080922f`及受限容量回归`b6f57d01`是回滚点。Phase3与UX/UI owner均确认本片IAM/Audit公共窗口无冲突；UI仍按其既定优先级独立工作，不提前消费候选接口。
...
2026-09-18，下列本地候选源码门禁及累计回归已通过；累计独立CI数据库作业尚未通过，S1b未宣告验收，失败及测试分组修正见[011运行约束](./FEAT-IAM-011-acceptance.md#运行约束)。真实PG18.4固定镜像限1CPU/1GiB/PIDs192，Linux Go1.26.5 runner限2CPU/1536MiB/PIDs256、GOMAXPROCS=2、GOMEMLIMIT=512MiB，串行`-race -p 1`；单项期限与生产密码成本未放宽。所有本地门禁终态成功、数据库无客户端且测试库已逐项释放后，已清理本轮专属临时PG及空网络；未操作其他任务资源。
