...
IAM API 按无状态、多实例设计；会话、凭据代际、附件、一次性意图与完成证据不能依赖单个进程内存或粘性路由。PAP 与 PDP 先保持同一部署边界，通过独立预算和有界并发避免登录密码计算、治理查询和策略管理挤占业务鉴权；没有负载证据不先拆出额外微服务。

权威读取使用能兑现已确认安全变更的数据库路径。撤权成功响应后开始的新请求，任意 IAM 副本都必须看到新状态；异步只读副本或缓存失效消息不能独立证明这一点。并发中的请求以明确的一致快照排序，不把“下一请求生效”解释为追溯取消已接受业务事务。数据库失联、切换未确认或必要状态缺失均失败关闭，不回退到旧 Allow。

缓存只允许优化不可变策略内容/解析结果，键绑定完整版本与 digest；它不能证明当前附件、默认版本、会话或主体仍有效，也不能替代本次决定持久化。热路径要有查询数、行数和策略求值预算，避免每个策略/组单独查询。连接池总量按实例数共同预算，队列/超时/重试均有上限；未知写入结果凭原幂等身份核对，不盲目重发。

账号公平性、入口全局预算与身份失败计数分别有明确作用域：单副本本地限流只能保护进程，不能宣称集群级账号配额或登录尝试上限。业务鉴权与高成本登录分别度量；outbox 投递可异步，但安全变更及事实持久化仍同事务，积压有监控与背压。

本轮验证单地域多 IAM 实例的路由无关性、撤权与过载隔离。数据库主备编排/真实切换需要安装 owner 的部署能力与独立故障域证据；尚未交付时明确标记缺口，不能把两个进程共享单库称为整套高可用。跨地域多活继续归 012 延期边界。容量档位必须绑定硬件、数据规模、策略复杂度、流量组合与错误率，由 011 的实测建立，不预填未经测量的 QPS 或可用性百分比。
...
本研究基线实际是 IAM4/Audit3/PaaS1 + r4；这不是其他分支的当前 profile，也不是本轮开发源码的可发布组合。最终组合按 011 的首版基线规则与安装 owner 重新冻结，不把开发中的联调版本提前当成最终版本。目录内部等价整理不提前修改 schema/readiness 数字。本分支的中间源码不能把 PaaS1 拼接成一个已验收、可安装的最终产品；最终组装必须采用固定的产品/安装消费者并通过完整 profile 门禁。

每片先聚焦，再真实 PG18 与独立服务进程；里程碑做相关回归。CI 精确 SHA 与本地真实证据各自记录。跨完整 release profile 不匹配仍在副作用前拒绝；SQL 可重放不证明旧 binary 或旧签名包可升级。

本次未上线 IAM 的首版 schema 基线与开发历史测试范围由 [011：未发布阶段与首版基线](./FEAT-IAM-011-acceptance.md#未发布阶段与首版基线) 唯一规定；开发中间版本不自动产生长期升级兼容义务。
