...
- 状态：实施中；结构诊断及自定义策略创建/读取/显式关联/真实授权首片已实现并通过本地真实门禁；完整版本管理、条件、边界和 UI 闭环未完成，整体未验收。
- 依赖：002、004、001 的目录。
...
当前开发 readiness 为 IAM10/Audit7/PaaS1，仅反映实际函数与封闭 action 契约；安装发布 profile 未修改，不能据此组装或宣称跨版本可升级。当前没有对外诊断 endpoint 或自定义策略 UI 验收。LANG-01 的改名/删除/版本增删切换、LANG-03 的受限通配、LANG-04 条件、LANG-05 边界、LANG-07 编辑器以及 LANG-08 完整委派仍必须继续实现，不能将此创建首片当作 FEAT Accepted。
