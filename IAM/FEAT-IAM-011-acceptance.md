...
修正仅将这五项按实际编译测试目录拆成串行进程，生产代码、测试体、数据库和资源限额不变。本地编译枚举核对9个顶层测试恰好分为5项一般事务、1项Role/STS和3项本人会话，各运行一次；工作流12个Bash入口语法检查通过，未声称YAML解析。使用真实编译清单对原Bash循环作命令替身检查，证明选择准确，枚举失败、空清单及测试失败均传播为非零退出；该流程检查不是数据库验收。修正后的独立CI须另外验证，不能回填原失败。
